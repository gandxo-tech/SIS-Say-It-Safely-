...
(function () {
  ...
  function computeStreak() {
    ...
    db.collection('users').doc(G.user.uid).get().then(function (doc) {
      ...
      var lastVisit = 0;
      if (d.lastVisit) {
        try { lastVisit = d.lastVisit.toMillis ? d.lastVisit.toMillis() : (d.lastVisit.seconds ? d.lastVisit.seconds * 1000 : 0); } catch(e) {}
      }
  ...
  function loadFeed(reset) {
    ...
    query.get().then(function (snap) {
      ...
      // allLoaded basé sur le nombre de docs Firestore reçus — PAS sur les posts filtrés
      if (snap.docs.length < POSTS_PER_BATCH) G.allLoaded = true;
      ...
      snap.forEach(function (doc) {
        var data = doc.data();
        data.id  = doc.id;
        // Filtrer isExpired et ancienneté côté client sans casser la pagination
        if (data.isExpired) return;
        if (data.createdAt && data.createdAt.toMillis && data.createdAt.toMillis() < thirtyDaysAgo) return;
        // Filtres par onglet — 100% côté client, zéro index Firestore requis
        if (G.feedTab === 'viral'    && (data.heatScore || 0) < 50) return;
        if (G.feedTab === 'csdpm'    && data.type !== 'csdpm')       return;
        if (G.feedTab === 'whispers' && data.type !== 'whisper')     return;
        if (G.feedTab === 'battles'  && data.type !== 'battle')      return;
        rawPosts.push(data);
      });
      ...
      rawPosts.forEach(function (post, idx) {
        try {
          ...
          console.warn('[SIS] Post ignoré dans loadFeed:', e.message);
        }
      });
      ...
      hide('loadMoreSpinner');
      hideSkeletons();
      console.error('[SIS] Erreur loadFeed:', err.code, err.message);
      if (err.message && err.message.indexOf('index') !== -1) {
        showToast('Index Firestore manquant — voir console F12', 'error');
        console.warn('[SIS] Crée l\'index ici :', err.message.split('here: ')[1] || '(voir message complet)');
      } else if (err.code === 'permission-denied') {
        showToast('Permission refusée — vérifie tes règles Firestore', 'error');
      } else {
        showToast('Erreur de chargement : ' + (err.code || err.message), 'error');
      }
    });
  }

  function buildFeedQuery() {
    // UNE SEULE query : orderBy('createdAt') uniquement
    // Zéro index composite requis — fonctionne immédiatement sur un projet neuf
    // Le filtrage par type/viral/battles se fait entièrement côté client dans loadFeed
    var q = db.collection('posts').orderBy('createdAt', 'desc');
    if (G.lastVisible) q = q.startAfter(G.lastVisible);
    q = q.limit(POSTS_PER_BATCH * 2);
    return q;
  }
  ...
  function variableReward(posts) {
    ...
    var result = [];
    var maxIter = posts.length * typeOrder.length + 10;
    var i = 0;
    while (result.length < posts.length && i < maxIter) {
    ...
    // Ajouter les posts non encore placés (types inconnus ou dépassement)
    typeOrder.forEach(function(t) {
      byType[t].forEach(function(p) { if (result.indexOf(p) === -1) result.push(p); });
    });
  ...
  function updateHeatScore(postId, post) {
    var _hr   = post.reactions || {};
    var total = ((+(_hr.fire)  || 0)
               + (+(_hr.skull) || 0)
               + (+(_hr.cry)   || 0)
               + (+(_hr.zap)   || 0));
  ...
  function renderPost(post, idx) {
    try {
      ...
      console.warn('[SIS] renderPost ignoré (post id=' + (post && post.id) + '):', e.message);
    }
  }
  ...
  function renderBattle(post) {
    var opts = Array.isArray(post.battleOptions) ? post.battleOptions : [];
  ...
  function renderPoll(post) {
    var opts = Array.isArray(post.pollOptions) ? post.pollOptions : [];
    var total = opts.reduce(function (s, o) { return s + (o.votes || 0); }, 0);
  ...
  function renderThread(post) {
    var parts = Array.isArray(post.parts) && post.parts.length ? post.parts : [post.content || ''];
    var html  = '';
  ...
  function renderHashtags(post) {
    var tags = Array.isArray(post.hashtags) ? post.hashtags : [];
  ...
  function initBurnPost(el, post) {
    ...
    var interval = setInterval(function () {
      ...
      // Autodestruction locale (affichage uniquement)
      var views = post.viewCount || 0;
      if (remaining <= 0 || views >= maxViews) {
        clearInterval(interval);
        burnDestroy(el, post.id, post.authorId);
      }
    }, 1000);
  }

  function burnDestroy(el, postId, authorId) {
    ...
    // Seul l'auteur peut marquer le post comme expiré dans Firestore
    // Les autres ne font que masquer localement
    if (G.user && G.user.uid === authorId) {
  ...
  function initBattlePost(el, post) {
    ...
    var unsub = db.collection('posts').doc(post.id).onSnapshot(function (snap) {
      ...
      var d    = snap.data();
      var opts = Array.isArray(d.battleOptions) ? d.battleOptions : [];
      var total = opts.reduce(function (s, o) { return s + (o.votes || 0); }, 0);
      opts.forEach(function (opt) {
        var pct    = total > 0 ? Math.round((opt.votes || 0) / total * 100) : 0;
  ...
  function initPollPost(el, post) {
    ...
    var unsub = db.collection('posts').doc(post.id).onSnapshot(function (snap) {
      if (!snap.exists) return;
      var d    = snap.data();
      var opts = Array.isArray(d.pollOptions) ? d.pollOptions : [];
      var total = opts.reduce(function (s, o) { return s + (o.votes || 0); }, 0);
      opts.forEach(function (opt) {
        var pct   = total > 0 ? Math.round((opt.votes || 0) / total * 100) : 0;
        var optEl = el.querySelector('[data-optid="'+opt.id+'"]');
  ...
  function handleReaction(el, post, reactionKey) {
    ...
    reactorRef.get().then(function (doc) {
      ...
      batch.commit().then(function () {
        ...
        // Mettre à jour le Heat Score après chaque réaction
        db.collection('posts').doc(post.id).get().then(function (snap) {
          if (snap.exists) {
            updateHeatScore(post.id, snap.data());
          }
        });
  ...
  function openComments(post) {
    ...
    // Listener temps réel — désabonner l'ancien avant d'en créer un nouveau
    if (currentCommentUnsub) {
      currentCommentUnsub();
    ...
    // NE PAS pusher dans G.listeners — currentCommentUnsub est nettoyé à chaque appel
    // et à la fermeture du modal
  }
  ...
  function sendComment() {
    ...
    var isFirstReactor = !post.firstReactor;
  ...
  function openPacte(callback) {
    ...
    loadRandomPost(function (post) {
      G.currentPostForAction = post ? post.id : null;
  ...
  function loadRandomPost(cb) {
    // Pas de filtre isExpired pour éviter l'index composite Firestore
    var randomId = db.collection('posts').doc().id;
    db.collection('posts')
      ...
      .then(function (snap) {
        if (!snap.empty) {
          ...
          if (data.isExpired) { cb(null); return; }
          cb(data);
          return;
        }
        // Si rien trouvé vers le haut, prendre le premier post
        db.collection('posts')
          .orderBy(firebase.firestore.FieldPath.documentId())
          .limit(1)
          .get()
          .then(function (snap2) {
            if (snap2.empty) { cb(null); return; }
            var data2 = snap2.docs[0].data();
            data2.id  = snap2.docs[0].id;
            if (data2.isExpired) { cb(null); return; }
            cb(data2);
  ...
  function publishPost() {
    if (!G.user) return;
    var text = (qs('#composerText') ? qs('#composerText').value : '').trim();
  ...
  function doPublishPost(text) {
    var hashtags = extractHashtags((qs('#hashtagInput') ? qs('#hashtagInput').value : '') + ' ' + text);
    ...
    if (G.composerType === 'burn') {
      ...
      postData.expiresAt = firebase.firestore.Timestamp.fromDate(new Date(Date.now() + G.burnTimer * 1000));
    }
    if (G.composerType === 'battle') {
      ...
      postData.battleEndsAt  = firebase.firestore.Timestamp.fromDate(new Date(Date.now() + G.battleDuration * 1000));
    }
    if (G.composerType === 'poll') {
      ...
      postData.pollEndsAt  = firebase.firestore.Timestamp.fromDate(new Date(Date.now() + G.pollDuration * 1000));
    }
  ...
  function resetComposer() {
    ...
    var spi = qs('#storyPhotoInput');
    if (spi) spi.removeAttribute('data-url');
    ...
    reader.onload = function (e) {
      ...
      img.onload = function () {
        ...
        if (canvas.toBlob) {
          ...
          // Fallback pour anciens WebViews Android
  ...
  function renderCurrentStory() {
    var group = G.storyList[G.storyGroupIdx];
    if (!group) { closeStoryViewer(); return; }
    var item = group.items[G.storyItemIdx];
    if (!item) { closeStoryViewer(); return; }
    G.storyIndex = G.storyGroupIdx; // Compat
    ...
    G.storyList.forEach(function (g, idx) {
      ...
      if (idx < G.storyGroupIdx) fill.style.width = '100%';
      bar.appendChild(fill);
      barsEl.appendChild(bar);
    });

    // Animer la barre courante
    var currentBar = barsEl.children[G.storyGroupIdx];
  ...
  function storyPrev() {
    ...
    if (G.storyItemIdx > 0) {
      G.storyItemIdx--;
      renderCurrentStory();
    } else if (G.storyGroupIdx > 0) {
      G.storyGroupIdx--;
      var prevGroup = G.storyList[G.storyGroupIdx];
      G.storyItemIdx = prevGroup ? Math.max(0, prevGroup.items.length - 1) : 0;
      renderCurrentStory();
    } else {
      closeStoryViewer();
    }
  ...
  function storyNext() {
    clearTimeout(G.storyTimer);
    var group = G.storyList[G.storyGroupIdx];
    if (group && G.storyItemIdx < group.items.length - 1) {
      G.storyItemIdx++;
      renderCurrentStory();
    } else if (G.storyGroupIdx < G.storyList.length - 1) {
      G.storyGroupIdx++;
      G.storyItemIdx = 0;
      renderCurrentStory();
    } else {
  ...
  function publishStory() {
    ...
    var photoUrl = qs('#storyPhotoInput') ? (qs('#storyPhotoInput').getAttribute('data-url') || '') : '';
    var storyData = {
      type:          type,
      content:       text,
      bg:            bg,
      mediaUrl:      type === 'media' ? photoUrl : '',
      authorId:      G.user.uid,
      authorDisplay: G.user.displayName,
      createdAt:     firebase.firestore.FieldValue.serverTimestamp(),
      expiresAt:     expiresAt
    };
    if (type === 'media' && !photoUrl) { showToast('Photo non encore uploadée', 'info'); return; }
  ...
  function checkNewPosts() {
    ...
    var unsub = db.collection('posts')
      ...
      .onSnapshot(function (snap) {
        var added = snap.docChanges().filter(function(c){ return c.type === 'added'; });
        if (!added.length) return;
        G.newPostsQueue += added.length;
    ...
    db.collection('posts')
      ...
      .then(function (snap) {
        ...
        top5.forEach(function (p, i) {
          ...
          var r    = p.reactions || {};
          var total = (r.fire||0) + (r.skull||0) + (r.cry||0) + (r.zap||0);
    ...
    on('#notifClear', 'click', function () {
      if (G.user) {
        ...
        localNotifications.forEach(function(n) {
          if (n.firestoreId) {
            batch.update(
              db.collection('users').doc(G.user.uid).collection('notifications').doc(n.firestoreId),
              { read: true }
            );
          }
        });
  ...
  function bindStories() {
    ...
    on('#storyPhotoInput', 'change', function (e) {
      ...
      if (!file) return;
      uploadToCloudinary(file, 'storyUploadProgress', 'storyUploadFill', 'storyUploadLabel', function (err, url) {
        if (err) { showToast('Erreur upload', 'error'); return; }
  ...
  function timeAgo(ts) {
    ...
    else if (ts && ts.seconds) { ms = ts.seconds * 1000; } // Firestore Timestamp plain object
    else { return ''; }
  ...
  function clearPosts() {
    readerIntervals.forEach(function(id){ clearInterval(id); });
    readerIntervals = [];
    var list = qs('#postsList');
  ...
  /* ══════════════════════════════════════════════════════════
     AUTH INLINE — Connexion / Inscription / Anonyme / Reset
  ══════════════════════════════════════════════════════════ */
  ...
  var authAnonStep = { login: 0, register: 0 };

  function authMsg(err) {
    return AUTH_ERRORS[err.code] || ('Erreur : ' + err.message);
  }

  function authSetErr(id, msg) {
    var el = qs('#' + id);
    if (!el) return;
    el.textContent = msg || '';
    var group = el.parentElement;
    var inp   = group ? group.querySelector('.auth-input') : null;
    if (inp) { msg ? inp.classList.add('auth-input-error') : inp.classList.remove('auth-input-error'); }
  }

  function authClearErrs(ids) { ids.forEach(function (id) { authSetErr(id, ''); }); }

  function authSetLoading(btnId, spinnerId, iconId, loading) {
    var btn = qs('#' + btnId); var sp = qs('#' + spinnerId); var ic = qs('#' + iconId);
    if (btn) btn.disabled = loading;
    if (sp)  loading ? sp.classList.remove('hidden')  : sp.classList.add('hidden');
    if (ic)  loading ? ic.classList.add('hidden')     : ic.classList.remove('hidden');
  }

  function authCreateProfile(uid, isAnon) {
    return db.collection('users').doc(uid).get().then(function (doc) {
      if (!doc.exists) {
        return db.collection('users').doc(uid).set({
          displayName:        generateAnonName(uid),
          isAnonymousAccount: isAnon,
          ghostMode:          false,
          level:              1,
          points:             0,
          streak:             0,
  ...
  window.authSwitchTab = function (tab) {
    ...
    var fLogin  = qs('#authFormLogin');  var fReg  = qs('#authFormRegister');
    var tLogin  = qs('#tabLogin');       var tReg  = qs('#tabRegister');
    if (fLogin) isLogin ? fLogin.classList.remove('hidden')       : fLogin.classList.add('hidden');
    if (fReg)   isLogin ? fReg.classList.add('hidden')            : fReg.classList.remove('hidden');
    if (tLogin) isLogin ? tLogin.classList.add('auth-tab-active') : tLogin.classList.remove('auth-tab-active');
    if (tReg)   isLogin ? tReg.classList.remove('auth-tab-active'): tReg.classList.add('auth-tab-active');
    authAnonStep = { login: 0, register: 0 };
  };

  window.authTogglePw = function (inputId, btn) {
    var input = qs('#' + inputId); if (!input) return;
  ...
  window.authDoLogin = function () {
    authClearErrs(['loginEmailErr', 'loginPasswordErr']);
    var email = (qs('#loginEmail').value || '').trim();
    var pw    = (qs('#loginPassword').value || '');
    if (!email)                           { authSetErr('loginEmailErr', 'Email requis'); return; }
    if (!/\S+@\S+\.\S+/.test(email))      { authSetErr('loginEmailErr', 'Email invalide'); return; }
    if (!pw)                              { authSetErr('loginPasswordErr', 'Mot de passe requis'); return; }
    authSetLoading('btnLogin', 'spinnerLogin', 'iconLogin', true);
    auth.signInWithEmailAndPassword(email, pw)
      .then(function () { showToast('Connexion réussie ✓', 'success'); })
      .catch(function (err) {
        authSetLoading('btnLogin', 'spinnerLogin', 'iconLogin', false);
        var msg = authMsg(err); authSetErr('loginEmailErr', msg); showToast(msg, 'error');
      });
  };

  window.authDoRegister = function () {
    authClearErrs(['regEmailErr', 'regPasswordErr', 'regConfirmErr']);
    var email   = (qs('#regEmail').value || '').trim();
    var pw      = (qs('#regPassword').value || '');
    var confirm = (qs('#regConfirm').value || '');
    if (!email)                           { authSetErr('regEmailErr', 'Email requis'); return; }
    if (!/\S+@\S+\.\S+/.test(email))      { authSetErr('regEmailErr', 'Email invalide'); return; }
    if (!pw)                              { authSetErr('regPasswordErr', 'Mot de passe requis'); return; }
    if (pw.length < 8)                    { authSetErr('regPasswordErr', 'Minimum 8 caractères'); return; }
    if (!confirm)                         { authSetErr('regConfirmErr', 'Confirme ton mot de passe'); return; }
    if (pw !== confirm)                   { authSetErr('regConfirmErr', 'Mots de passe différents'); return; }
    authSetLoading('btnRegister', 'spinnerRegister', 'iconRegister', true);
    auth.createUserWithEmailAndPassword(email, pw)
      .then(function (cred) { return authCreateProfile(cred.user.uid, false); })
      .then(function ()      { showToast('Compte créé ✓ Bienvenue !', 'success'); })
      .catch(function (err) {
        authSetLoading('btnRegister', 'spinnerRegister', 'iconRegister', false);
        var msg = authMsg(err); authSetErr('regEmailErr', msg); showToast(msg, 'error');
      });
  };

  window.authDoAnonymous = function (form) {
    var infoEl = qs('#anonInfo' + (form === 'login' ? 'Login' : 'Register'));
    if (authAnonStep[form] === 0) {
      if (infoEl) infoEl.classList.remove('hidden');
      authAnonStep[form] = 1; return;
    }
    auth.signInAnonymously()
      .then(function (cred) { return authCreateProfile(cred.user.uid, true); })
      .then(function ()      { showToast('Entrée anonyme ✓', 'success'); })
      .catch(function (err) { authAnonStep[form] = 0; showToast('Erreur : ' + err.message, 'error'); });
  };

  window.authOpenReset = function () {
    var overlay = qs('#resetOverlay'); if (overlay) overlay.classList.remove('hidden');
    var email = (qs('#loginEmail').value || '').trim();
    var inp   = qs('#resetEmail'); if (email && inp) inp.value = email;
  };

  window.authCloseReset = function () {
    var overlay = qs('#resetOverlay'); if (overlay) overlay.classList.add('hidden');
    authSetErr('resetEmailErr', '');
  };

  window.authDoReset = function () {
    authSetErr('resetEmailErr', '');
    var email = (qs('#resetEmail').value || '').trim();
    if (!email)                      { authSetErr('resetEmailErr', 'Email requis'); return; }
    if (!/\S+@\S+\.\S+/.test(email)) { authSetErr('resetEmailErr', 'Email invalide'); return; }
    authSetLoading('btnReset', 'spinnerReset', 'iconReset', true);
    auth.sendPasswordResetEmail(email)
      .then(function () {
        authSetLoading('btnReset', 'spinnerReset', 'iconReset', false);
        authCloseReset(); showToast('Email envoyé ! Vérifie ta boîte.', 'success');
      })
      .catch(function (err) {
        authSetLoading('btnReset', 'spinnerReset', 'iconReset', false);
        authSetErr('resetEmailErr', authMsg(err));
      });
  };

  function initAuthListeners() {
    /* Barre force mot de passe */
    var regPwEl = qs('#regPassword');
    if (regPwEl) {
      regPwEl.addEventListener('input', function () {
        var pw = this.value; var score = 0;
        if (pw.length >= 8)           score++;
        if (/[A-Z]/.test(pw))         score++;
        if (/[0-9]/.test(pw))         score++;
        if (/[^A-Za-z0-9]/.test(pw))  score++;
        var colors = ['', '#EF4444', '#F59E0B', '#5B8EF4', '#10B981'];
        for (var i = 1; i <= 4; i++) {
          var bar = qs('#sb' + i);
          if (bar) bar.style.background = i <= score ? colors[score] : 'var(--bg4)';
        }
      });
    }
    /* Fermer reset overlay hors modal */
    var resetOv = qs('#resetOverlay');
    if (resetOv) {
      resetOv.addEventListener('click', function (e) {
        if (e.target === resetOv) authCloseReset();
      });
    }
    ...
    document.addEventListener('keydown', function (e) {
      ...
      if (id === 'loginEmail' || id === 'loginPassword')                          window.authDoLogin();
      else if (id === 'regEmail' || id === 'regPassword' || id === 'regConfirm')  window.authDoRegister();
      else if (id === 'resetEmail')                                               window.authDoReset();
    });
  }
