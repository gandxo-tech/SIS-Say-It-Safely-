...
var sisState = {
  ...
  storyTimer:     null,
  notifUnread:    0,
  unsubscribers:  [],     // listeners Firestore à détacher
  readerInterval: null,
  vibeInterval:   null,
  liveInterval:   null,
  burnIntervals:  {}
...
function buildQuery() {
  ...
  var q   = col.where('isExpired', '==', false);

  // Filtre par tab
  if (sisState.currentTab === 'viral') {
    q = col.where('isExpired','==',false).orderBy('heatScore','desc');
  } else if (sisState.currentTab === 'csdpm') {
    q = col.where('type','==','csdpm').where('isExpired','==',false);
  } else if (sisState.currentTab === 'whispers') {
    q = col.where('type','==','whisper').where('isExpired','==',false);
  } else if (sisState.currentTab === 'battles') {
    q = col.where('type','==','battle').where('isExpired','==',false);
  } else if (sisState.currentTab === 'following') {
    // Pour l'instant : même chose que global
    q = col.where('isExpired','==',false);
  }

  // Filtre mood
  if (sisState.currentMood) {
    q = q.where('topic', '==', sisState.currentMood);
  }

  // Filtre tag
  if (sisState.currentTag) {
    q = q.where('hashtags','array-contains', sisState.currentTag);
  }

  q = q.orderBy('createdAt', 'desc').limit(POSTS_PER_BATCH);
...
function startBattleTimer(post) {
  ...
  sisState.unsubscribers.push(function() { clearInterval(interval); });
}
...
function handleReaction(postId, type, btn) {
  ...
  postRef.get().then(function(snap) {
    ...
    if (!data.firstReactor) {
      update.firstReactor = uid;
    }
...
function voteBattle(postId, optIdx, postEl) {
  if (!sisState.user) return;
  var sessionKey = 'sis-battle-' + postId;
  ...
  db.collection('posts').doc(postId).update(update)
    .then(function() {
      return db.collection('posts').doc(postId).get();
    })
    .then(function(snap) {
...
function startBurnTimer(post) {
  ...
  sisState.unsubscribers.push(function() { clearInterval(sisState.burnIntervals[id]); });
}
...
function registerView(postId) {
  db.collection('posts').doc(postId).get().then(function(snap) {
    ...
    var newCount = (data.viewCount || 0) + 1;
    ...
    db.collection('posts').doc(postId).update({
      ...
      if (viewsEl) viewsEl.textContent = (maxViews - newCount) + ' vues restantes';
...
function openPacte() {
  ...
  db.collection('posts')
    ...
    .then(function(snap) {
      if (snap.empty) {
        // Si pas encore de posts, publier directement
      ...
      document.querySelectorAll('.pacte-er').forEach(function(btn) {
        btn.addEventListener('click', function pacteClick() {
          // Réagir au post aléatoire
          var update = {};
          ...
          // Publier le post en attente
          closeModal('pacteModal');
          publishPost(sisState.pendingPostData);

          // Retirer le listener pour éviter double publication
          btn.removeEventListener('click', pacteClick);
        }, { once: true });
      });
    })
    .catch(function() {
...
function startReaderPing() {
  function ping() {
    ...
    var uid = sisState.user.uid;
    // Ping sur tous les posts visibles
