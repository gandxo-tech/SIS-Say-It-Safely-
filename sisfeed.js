...
(function () {
  ...
  function handleReaction(el, post, reactionKey) {
    ...
    reactorRef.get().then(function (doc) {
      ...
      if (prev === reactionKey) {
        // Toggle off
        updates[reactionKey] = firebase.firestore.FieldValue.increment(-1);
        batch.update(db.collection('posts').doc(post.id), { ['reactions.'+reactionKey]: firebase.firestore.FieldValue.increment(-1) });
        batch.delete(reactorRef);
      } else {
        if (prev) {
          batch.update(db.collection('posts').doc(post.id), { ['reactions.'+prev]: firebase.firestore.FieldValue.increment(-1) });
        }
        batch.update(db.collection('posts').doc(post.id), { ['reactions.'+reactionKey]: firebase.firestore.FieldValue.increment(1) });
  ...
  function closePacte(reactionKey) {
    ...
    if (reactionKey && G.currentPostForAction) {
      db.collection('posts').doc(G.currentPostForAction).update({
        ['reactions.'+reactionKey]: firebase.firestore.FieldValue.increment(1)
      }).catch(function () {});
    }
    if (G.pacteCallback) {
      G.pacteCallback();
