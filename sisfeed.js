...
(function () {
  ...
  function initAuth() {
    auth.onAuthStateChanged(function (user) {
      if (!user) {
        // Afficher l'overlay auth inline
        var overlay = qs('#authOverlay');
        if (overlay) overlay.classList.remove('hidden');
        return;
      }
      // Masquer l'overlay auth si visible
      var overlay = qs('#authOverlay');
  ...
  function bindProfilPanel() {
    ...
    on('#ppLogoutBtn', 'click', function () {
      ...
      auth.signOut().then(function () {
        // Recharger la page pour tout nettoyer proprement
  ...
  function authSetErr(id, msg) {
    ...
    if (!el) return;
    el.textContent = msg || '';
    /* Trouver le champ input parent dans le même form-group */
  ...
  function authSetLoading(btnId, spinnerId, iconId, loading) {
    var btn     = qs('#'+btnId);
    var spinner = qs('#'+spinnerId);
    var icon    = qs('#'+iconId);
    if (btn)     btn.disabled       = loading;
    if (spinner) loading ? spinner.classList.remove('hidden') : spinner.classList.add('hidden');
    if (icon)    loading ? icon.classList.add('hidden')       : icon.classList.remove('hidden');
  }
  ...
  window.authSwitchTab = function (tab) {
    if (tab === 'login') {
      qs('#authFormLogin').classList.remove('hidden');
      qs('#authFormRegister').classList.add('hidden');
      qs('#tabLogin').classList.add('auth-tab-active');
      qs('#tabRegister').classList.remove('auth-tab-active');
    } else {
      qs('#authFormLogin').classList.add('hidden');
      qs('#authFormRegister').classList.remove('hidden');
      qs('#tabRegister').classList.add('auth-tab-active');
      qs('#tabLogin').classList.remove('auth-tab-active');
    }
  ...
  /* Barre de force — initialisée dans initAuthListeners() */
  ...
  function initAuthListeners() {
    ...
    /* Fermer reset overlay en cliquant hors de la modal */
    var resetOvEl2 = qs('#resetOverlay');
    if (resetOvEl2) {
      resetOvEl2.addEventListener('click', function (e) {
        if (e.target === resetOvEl2) authCloseReset();
      });
    }

    /* Soumission au Enter dans les champs auth */
    document.addEventListener('keydown', function (e) {
      if (e.key !== 'Enter') return;
      var activeId = document.activeElement && document.activeElement.id;
      if (activeId === 'loginEmail' || activeId === 'loginPassword')
        window.authDoLogin();
      else if (activeId === 'regEmail' || activeId === 'regPassword' || activeId === 'regConfirm')
        window.authDoRegister();
      else if (activeId === 'resetEmail')
        window.authDoReset();
    });
  }
  ...
  function authFirebaseMsg(err) {
    return AUTH_ERRORS[err.code] || ('Erreur : ' + err.message);
  }

  function authCreateProfile(uid, isAnonymous) {
  ...
  window.authDoLogin = function () {
    ...
    if (!email)                          { authSetErr('loginEmailErr','Email requis'); ok = false; }
    else if (!/\S+@\S+\.\S+/.test(email)){ authSetErr('loginEmailErr','Email invalide'); ok = false; }
    if (!pw)                             { authSetErr('loginPasswordErr','Mot de passe requis'); ok = false; }
    if (!ok) return;
  ...
  window.authDoRegister = function () {
    ...
    if (!email)                           { authSetErr('regEmailErr','Email requis'); ok = false; }
    else if (!/\S+@\S+\.\S+/.test(email)) { authSetErr('regEmailErr','Email invalide'); ok = false; }
    if (!pw)                              { authSetErr('regPasswordErr','Mot de passe requis'); ok = false; }
    else if (pw.length < 8)               { authSetErr('regPasswordErr','Minimum 8 caractères'); ok = false; }
    if (!confirm)                         { authSetErr('regConfirmErr','Confirme ton mot de passe'); ok = false; }
    else if (pw !== confirm)              { authSetErr('regConfirmErr','Les mots de passe ne correspondent pas'); ok = false; }
    if (!ok) return;
    ...
    auth.createUserWithEmailAndPassword(email, pw)
      .then(function (cred) {
        return authCreateProfile(cred.user.uid, false);
      })
      .then(function () {
  ...
  /* ANONYME — 2 clics : 1er = info, 2e = connexion */
  window.authDoAnonymous = function (form) {
    var infoId = form === 'login' ? 'anonInfoLogin' : 'anonInfoRegister';
    var infoEl = qs('#'+infoId);

    if (authAnonStep[form] === 0) {
      // Premier clic — afficher l'info
      if (infoEl) infoEl.classList.remove('hidden');
      authAnonStep[form] = 1;
      return;
    }

    // Deuxième clic — connecter
    auth.signInAnonymously()
      .then(function (cred) {
        return authCreateProfile(cred.user.uid, true);
      })
      .then(function () {
        showToast('Entrée anonyme ✓', 'success');
      })
      .catch(function (err) {
        showToast('Erreur anonyme : ' + err.message, 'error');
        authAnonStep[form] = 0;
      });
  };
  ...
  window.authDoReset = function () {
    ...
    auth.sendPasswordResetEmail(email)
      ...
      .catch(function (err) {
        authSetLoading('btnReset','spinnerReset','iconReset', false);
        authSetErr('resetEmailErr', authFirebaseMsg(err));
      });
  };
