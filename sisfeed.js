...
(function () {
  ...
  function loadFeed(reset) {
    ...
    query.get().then(function (snap) {
      ...
      snap.forEach(function (doc) {
        var data = doc.data();
        data.id  = doc.id;
        // Filtrer côté client : posts expirés et trop vieux
        if (data.isExpired) return;
        if (data.createdAt && data.createdAt.toMillis() < thirtyDaysAgo) return;
        // Filtre viral côté client
        if (G.feedTab === 'viral' && (data.heatScore || 0) < 50) return;
        // Filtre battles expirés
  ...
  function buildFeedQuery() {
    ...
    if (G.feedTab === 'viral') {
      // Pas de filtre isExpired → pas d'index composite requis
      ...
      // Un seul where pour éviter l'index composite
      q = col.where('type','==','battle')
             .orderBy('createdAt','desc');
    } else {
      // Global + Following : orderBy seul = index automatique
      q = col.orderBy('createdAt','desc');
    }

    if (G.lastVisible) q = q.startAfter(G.lastVisible);
    q = q.limit(POSTS_PER_BATCH * 2); // charger plus pour compenser le filtre client
    return q;
  }
  ...
  function loadStories() {
    ...
    // Charger d'abord les stories de l'utilisateur courant
    db.collection('stories').get().then(function (usersSnap) {
      var row = qs('#storiesRow');
      var createBtn = qs('#createStoryBtn');
      if (!row || !createBtn) return;
      ...
      var uids = [];
      usersSnap.forEach(function (doc) { uids.push(doc.id); });
      ...
      uids.forEach(function (uid) {
        db.collection('stories').doc(uid).collection('items')
          ...
          .then(function (snap) {
            ...
            if (!snap.empty) {
              ...
              storyGroups.push({ uid: uid, items: items });
            }
            // Une fois tous les UIDs traités, rendre les stories
            if (processed === uids.length) {
              storyGroups.forEach(function (group) {
                G.storyList.push(group);
                var item = group.items[0];
                var storyEl = document.createElement('div');
                storyEl.className = 'story-item';
                var avHtml = generateAvatar(group.uid, 46);
                var isMe   = group.uid === G.user.uid;
            ...
            processed++;
            console.warn('Stories uid ' + uid + ':', err.message);
          });
      });
    }).catch(function (err) {
      console.warn('loadStories:', err.message);
    });
  }
  ...
  function publishStory() {
    ...
    var text  = qs('#storyText') ? qs('#storyText').value.trim() : '';
    var bg    = qs('.story-bg-opt.story-bg-active') ? qs('.story-bg-opt.story-bg-active').getAttribute('data-bg') : 'bg1';
    var expiresAt = new Date(Date.now() + 86400000);

    if (type === 'text' && !text) { showToast('Écris quelque chose', 'info'); return; }

    // Récupérer l'URL photo cloudinary si type media
    var photoInput = qs('#storyPhotoInput');
    var mediaUrl   = photoInput ? photoInput.getAttribute('data-url') : null;
    if (type === 'media' && !mediaUrl) { showToast('Attends que la photo soit uploadée', 'info'); return; }
    ...
    db.collection('stories').doc(G.user.uid).collection('items').add(storyData)
      .then(function () {
        ...
        if (qs('#storyText')) qs('#storyText').value = '';
        if (photoInput) photoInput.removeAttribute('data-url');
        loadStories();
        showToast('Story publiée ✓', 'success');
      }).catch(function (err) {
        showToast('Erreur publication story', 'error');
        console.error(err);
      });
  }
  ...
  function authSetErr(id, msg) {
    ...
    if (inp) {
      msg ? inp.classList.add('auth-input-error') : inp.classList.remove('auth-input-error');
    }
  }

  function authClearErrs(ids) {
    ids.forEach(function (id) { authSetErr(id, ''); });
  }
  ...
  window.authSwitchTab = function (tab) {
    ...
    var fLogin  = qs('#authFormLogin');
    var fReg    = qs('#authFormRegister');
    var tLogin  = qs('#tabLogin');
    var tReg    = qs('#tabRegister');
  ...
  window.authOpenReset = function () {
    var overlay = qs('#resetOverlay');
    if (overlay) overlay.classList.remove('hidden');
    var email = (qs('#loginEmail').value || '').trim();
    var inp   = qs('#resetEmail');
    if (email && inp) inp.value = email;
  };
