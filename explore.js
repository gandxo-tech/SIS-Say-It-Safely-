...
function loadLives() {
    ...
    const unsubLive = onSnapshot(liveQuery, (snapshot) => {
        ...
        if (snapshot.empty) {
            ...
            snapshot.forEach(doc => {
                const card = createLiveCard(doc.id, doc.data(), true);
    ...
    const unsubScheduled = onSnapshot(scheduledQuery, (snapshot) => {
        ...
        if (snapshot.empty) {
            ...
            snapshot.forEach(doc => {
                const card = createLiveCard(doc.id, doc.data(), false);
                grid.appendChild(card);
            });
        }
    });
...
async function joinLive(liveId) {
    ...
    try {
        ...
        // Trigger live room setup (handled by live.js)
