  ...
  if (user) {
    try {
      ...
      if (doc.exists) {
        ...
        // Document pas encore créé (ex: juste après inscription)
        state.user = {
          uid: user.uid,
          name: user.displayName || 'Utilisateur',
          email: user.email,
          photo: user.photoURL || localStorage.getItem(`sis_photo_${user.uid}`) || null,
          isAnonymous: user.isAnonymous
        };
      }
      ...
      // En cas d'erreur Firestore, on affiche quand même l'app
      state.user = {
        uid: user.uid,
        name: user.displayName || 'Utilisateur',
        email: user.email,
