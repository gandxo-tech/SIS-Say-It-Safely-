...
(function () {
  ...
  function checkAdmin() {
    if (currentUserData.role === 'admin' || currentUserData.certified) {
      var sideAdmin = $('sideMenuAdmin');
      if (sideAdmin) show(sideAdmin);
      /* Note: bouton admin visible dans sideMenu uniquement */
    }
  }
  ...
  function showView(name) {
    ...
    /* Recharger la liste DM à chaque ouverture de l'onglet Messages */
  ...
  function initInputBar() {
    ...
    if (btnEmoji) {
      btnEmoji.addEventListener('click', function(e) {
        ...
        var isOpen = emojiPicker.style.display !== 'none' && emojiPicker.style.display !== '';
        if (isOpen) {
          hide(emojiPicker);
        } else {
          show(emojiPicker);
    ...
    if (btnAttach) {
      btnAttach.addEventListener('click', function(e) {
        ...
        var isOpen = attachMenu.style.display !== 'none' && attachMenu.style.display !== '';
        if (isOpen) { hide(attachMenu); } else { show(attachMenu); }
      });
    }

    document.addEventListener('click', function(e) {
      /* Ne pas fermer si clic à l'intérieur du picker ou du menu */
      if (attachMenu && !attachMenu.contains(e.target)) hide(attachMenu);
      if (emojiPicker && !emojiPicker.contains(e.target) && e.target !== btnEmoji) hide(emojiPicker);
    });
  ...
  function openRoomInfo(roomId, roomData) {
    ...
    if (rMbrs) rMbrs.textContent = '👥 ' + (d.membersOnline || 0) + ' membres';
    ...
    if (slowCtrl) {
      ...
      if (isAdmin) {
        ...
        if (smSelect) {
          smSelect.value = String(d.slowMode || 0);
        }
        if (btnApplySM) {
      ...
      .onSnapshot(function(snap) {
        ...
        docs.forEach(function(doc) {
          ...
          db.collection('users').doc(otherUid).get().then(function(uDoc) {
            ...
            var existing = list.querySelector('[data-dm-id="'+doc.id+'"]');
            if (!existing) {
              existing = document.createElement('div');
              existing.className = 'dm-item';
              existing.dataset.dmId = doc.id;
              list.appendChild(existing);
            }
            existing.classList.toggle('active', doc.id === currentDmId);
            existing.innerHTML =
  ...
  function startDmSession(dmId) {
    ...
    /* Read receipts sur les messages non lus */
    markDmMessagesRead(dmId);
  }
  ...
  function dmSendMessage(payload) {
    ...
    db.collection('dms').doc(currentDmId).collection('messages').add(msg)
      .then(function() {
        ...
        /* Notification Firestore pour l'autre */
        db.collection('notifications').add({
          toUid:     currentDmOtherUid,
          type:      'dm',
          text:      (d.pseudo||'Quelqu\'un') + ' : ' + preview,
          roomId:    '',
          dmId:      currentDmId,
  ...
  function animateDmWaveform() {
    ...
    for (var i=0;i<16;i++){
      var s = document.createElement('span');
      s.style.cssText = 'display:inline-block;width:3px;border-radius:2px;background:var(--accent-grad,#5B8EF4);height:'+(4+Math.floor(Math.random()*18))+'px;margin:0 1px;vertical-align:middle;';
      wf.appendChild(s);
    }
  }
  ...
  function restoreDmInputBar() {
    dmInputBound = false;
    var inputBar = $('dmInputBar');
    if (!inputBar) return;
    delete inputBar.dataset.recording;
    var textWrap = qs('#dmInputBar .input-text-wrap');
    if (textWrap) {
      textWrap.innerHTML =
        '<div id="dmMsgInput" class="msg-input" contenteditable="true" role="textbox" aria-label="Message privé" data-placeholder="Message privé…"></div>' +
        '<button class="icon-btn btn-emoji" id="btnDmEmoji">' +
          '<svg viewBox="0 0 22 22" fill="none" width="22" height="22"><circle cx="11" cy="11" r="8.5" stroke="currentColor" stroke-width="1.8"/><path d="M7.5 13.5s1 2 3.5 2 3.5-2 3.5-2" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/><circle cx="8.5" cy="9.5" r="1" fill="currentColor"/><circle cx="13.5" cy="9.5" r="1" fill="currentColor"/></svg>' +
        '</button>';
    }
    bindDmInputBar();
  }
  ...
  function initProfileView() {
    ...
    if (btnAvatarEdit && fileAvatar) {
      ...
      fileAvatar.addEventListener('change', function(e) {
        ...
        reader.onload = function(ev) {
          showToast('Upload en cours…', 'info', 15000);
          uploadToCloudinary(ev.target.result, function(url) {
            ...
            db.collection('users').doc(currentUser.uid).update({ avatarUrl: url })
              .then(function() {
                ...
                /* Rafraîchir TOUS les avatars (topbar, bnav, sideMenu, profil) */
                renderUserUI();
