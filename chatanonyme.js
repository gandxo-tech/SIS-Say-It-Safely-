...
(function () {
  ...
  var BOT_CMDS = [
    { cmd: '/anime',    args: '[titre]',  desc: 'Fiche complète animé (MAL)' },
  ...
  function bcFetchCitation(theme) {
    var tags = theme ? '?tags='+encodeURIComponent(theme) : '';
    return fetch('https://api.quotable.io/random' + tags)
      .then(function(r){ if(!r.ok) return fetch('https://api.quotable.io/random'); return r; })
      .then(function(r2){ return r2.json(); })
      .then(function(d) {
        if (!d.content) throw new Error('Citation introuvable');
        return {
          cardType: 'citation',
          label: '💬 Citation',
          text: d.content,
