...
(function () {
  ...
  function openRoom(roomId, roomData) {
    /* ── Fix #1 : décrémenter l'ancien salon si on en change directement ── */
  ...
  function initInputBar() {
    ...
    document.addEventListener('click', function(e) {
      ...
      if (emojiPicker && emojiPicker.style.display !== 'none') {
        /* Fix #4 : inclure btnDmEmoji pour ne pas fermer le picker DM au même tick */
        var btnDmEmoji = $('btnDmEmoji');
        var clickedInsidePicker  = emojiPicker.contains(e.target);
        var clickedRoomEmojiBtn  = btnEmoji && btnEmoji.contains(e.target);
        var clickedDmEmojiBtn    = btnDmEmoji && btnDmEmoji.contains(e.target);
        if (!clickedInsidePicker && !clickedRoomEmojiBtn && !clickedDmEmojiBtn) {
          hide(emojiPicker);
        }
      }
    });
  ...
  function isSpam(text) {
    /* Fix #8 : incrémenter avant le test pour bloquer dès le 3ème envoi identique */
    if (text === lastMsgText) {
      repeatCount++;
  ...
  function startEditMessage(msgId, data) {
    ...
    if (btnSend) {
      ...
      /* Fix #7 : stocker la ref de onKey pour pouvoir la retirer proprement */
      function cancelEdit() {
        input.textContent = '';
  ...
  function dmSendText() {
    ...
    if (!text) return;
    input.textContent = '';
    /* Fix #2 : remettre le bouton send/voice à l'état initial */
    var btnSend  = $('btnDmSend');
    var btnVoice = $('btnDmVoice');
    if (btnSend)  btnSend.style.display  = 'none';
    if (btnVoice) btnVoice.style.display = '';
  ...
  function markNotifsRead() {
    if (!currentUser) return;
    /* Bouton "Tout marquer comme lu" — lié une seule fois */
    var btnMarkAll = $('btnMarkAllRead');
    if (btnMarkAll && !btnMarkAll.dataset.bound) {
      btnMarkAll.dataset.bound = '1';
      /* Fix #6 : appeler la sous-fonction directement, pas markNotifsRead() */
  ...
  function doLogout() {
    ...
    db.collection('users').doc(currentUser.uid).update({ status:'offline', lastSeen: firebase.firestore.FieldValue.serverTimestamp() })
      .catch(function(){})
      .then(function() {
        /* Fix #9 : .finally() absent sur certains SDK compat → .then().catch() */
  ...
  ══════════════════════════════════════════════════════════════ */
  var globalSearchBound = false; /* Fix #3 : anti-empilement listener */
  ...
  function buildEmojiGrid(grid, targetInput) {
    ...
    EMOJIS_QUICK.forEach(function(emoji) {
      ...
      btn.addEventListener('click', function() {
        ...
        /* Fix #5 : cibler les bons boutons selon le contexte (salon ou DM) */
        var isDm    = targetInput && targetInput.id === 'dmMsgInput';
        var btnSend  = isDm ? $('btnDmSend')  : $('btnSend');
        var btnVoice = isDm ? $('btnDmVoice') : $('btnVoice');
        if (btnSend)  btnSend.style.display  = hasText ? '' : 'none';
        if (btnVoice) btnVoice.style.display = hasText ? 'none' : '';
      });
  ...
  function boot() {
    ...
    if (modalSearch) {
      ...
      var observer = new MutationObserver(function() {
        if (modalSearch.style.display !== 'none' && !globalSearchBound) {
