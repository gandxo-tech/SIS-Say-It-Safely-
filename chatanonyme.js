...
try {
  ...
  // Firebase failed to init — show error on screen
  document.addEventListener('DOMContentLoaded', function() {
    var pa = document.getElementById('pageAuth');
    if (pa) { pa.style.display = 'flex'; pa.classList.add('active'); }
  });
...
var S = {
  ...
  stickers: (function(){ try{ return JSON.parse(localStorage.getItem('sis_stickers') || '[]'); }catch(e){ return []; } })(),
  notifs:   (function(){ try{ return JSON.parse(localStorage.getItem('sis_notifs')   || '[]'); }catch(e){ return []; } })(),
...
(function boot() {
  ...
  initCryptoKey()
    .catch(function() { S.cryptoKey = null; })
    .then(function() {
...
function onAppReady() {
  ...
  // Mobile nav avatar
  var mbnAv = document.getElementById('mbnAvatarEl');
  if (mbnAv) {
    if ((u && u.photoURL) || (p && p.photoURL)) {
      mbnAv.innerHTML = '<img src="' + (u && u.photoURL || p && p.photoURL) + '" style="width:100%;height:100%;object-fit:cover;border-radius:50%"/>';
      mbnAv.style.background = 'none';
    } else if (u && u.isAnonymous && p && p.anonEmoji) {
      mbnAv.textContent = p.anonEmoji;
      mbnAv.style.background = p.anonColor || '#6c63ff';
    } else {
      mbnAv.textContent = ((p && p.displayName) || 'U')[0].toUpperCase();
      mbnAv.style.background = 'var(--grad)';
    }
  }
...
// ─────────────────────────────────────────────
//  PRÉSENCE — actif UNIQUEMENT quand l'onglet est visible
// ─────────────────────────────────────────────
function setupPresence() {
  ...
  // Marquer online/offline selon visibilité réelle de l'onglet
  function setOnline() {
    if (!S.user) return;
    ref.set({ status: 'online', lastSeen: firebase.firestore.FieldValue.serverTimestamp() }, { merge: true }).catch(function(){});
  }
  function setOfflinePresence() {
    if (!S.user) return;
    ref.set({ status: 'offline', lastSeen: firebase.firestore.FieldValue.serverTimestamp() }, { merge: true }).catch(function(){});
  }

  // Appliquer l'état initial selon la visibilité actuelle
  ...
  // Écouter les changements de visibilité (onglet caché/actif, alt+tab, verrouillage)
  ...
  S.listeners.push(function() { document.removeEventListener('visibilitychange', onVisChange); });

  // Heartbeat toutes les 30s (uniquement si l'onglet est actif)
  var hb = setInterval(function() {
    if (!S.user) return;
    if (document.visibilityState === 'visible') {
      ref.set({ lastSeen: firebase.firestore.FieldValue.serverTimestamp() }, { merge: true }).catch(function(){});
    }
  }, 30000);
  S.listeners.push(function() { clearInterval(hb); });

  // Offline à la fermeture
...
function fetchCountryFlag() {
  ...
  try {
    ...
    var _timer = setTimeout(function() { try { _ctrl.abort(); } catch(e){} }, 3000);
    fetch('https://ipapi.co/json/', { signal: _ctrl.signal })
      ...
      }).catch(function() { clearTimeout(_timer); });
  } catch(e) {}
}
...
function loadNotifBadge() {
  ...
  // Mobile nav badge
  var mbnBadge = document.getElementById('mbnNotifBadge');
  if (mbnBadge) {
    if (unread > 0) { mbnBadge.textContent = unread > 9 ? '9+' : unread; mbnBadge.style.display = 'flex'; }
  ...
  uploadCloud(file, 'image').then(function(url) {
    return S.user.updateProfile({ photoURL: url })
      ...
      .then(function() {
        ...
        // Mobile nav avatar
        var mbnAv = document.getElementById('mbnAvatarEl');
        if (mbnAv) { mbnAv.innerHTML = '<img src="' + url + '" style="width:100%;height:100%;object-fit:cover;border-radius:50%"/>'; mbnAv.style.background = 'none'; }
...
function switchView(name) {
  ...
  var mbnMap = { rooms: 'mbnRooms', dms: 'mbnMessages', random: 'mbnRandom', admin: null };
  document.querySelectorAll('.mbn-btn').forEach(function(b) { b.classList.remove('active'); });
  var mbnId = mbnMap[name];
  if (mbnId) { var mbnBtn = document.getElementById(mbnId); if (mbnBtn) mbnBtn.classList.add('active'); }

  if (name === 'admin') loadAdminStats();
  if (name === 'dms') loadDmList();
}

// Mobile bottom nav — même logique que switchView
...
// ─────────────────────────────────────────────
function exportChat() {
  // Feature supprimée à la demande de l'utilisateur
  toast('Export non disponible.', 'info');
}
...
Object.assign(window, {
  ...
  // Thème
  setMode, setChatBg, uploadChatBg, applyChatBg, setCustomBg,
  // Rooms
  ...
  // Ephem
  openEphemModal, pickEphem, setEphem, confirmEphem,
  // Admin
  ...
  closeSidebarMobile,
  // DM view (vue Mes Messages)
  loadDmList, filterDmList, closeDmChat,
});

// ═══════════════════════════════════════════════════════════════
//  NOUVELLES FONCTIONS — SIS V4.2
//  Navigation mobile, Vue DM, Aliases
// ═══════════════════════════════════════════════════════════════

// ─────────────────────────────────────────────
//  VUE "MES MESSAGES" — Liste conversations DM
// ─────────────────────────────────────────────
var dmListUnsub = null;

function loadDmList() {
  if (!S.user) return;
  var list = document.getElementById('dmConvList');
  if (!list) return;
  list.innerHTML = '<div style="padding:20px;text-align:center;color:var(--t2)">Chargement…</div>';

  if (dmListUnsub) dmListUnsub();
  dmListUnsub = db.collection('dms')
    .where('participants', 'array-contains', S.user.uid)
    .orderBy('updatedAt', 'desc').limit(30)
    .onSnapshot(function(snap) {
      if (snap.empty) {
        list.innerHTML = '<div class="empty-state">' + SVG.chat + '<span>Aucune conversation</span></div>';
        return;
      }
      // Fetch les noms des interlocuteurs
      var promises = snap.docs.map(function(d) {
        var data = d.data();
        var otherId = (data.participants || []).find(function(uid) { return uid !== S.user.uid; });
        if (!otherId) return Promise.resolve(null);
        return db.collection('users').doc(otherId).get().then(function(uSnap) {
          ...
          return {
            ...
            otherId:     otherId,
            name:        u.displayName || 'Anonyme',
            photoURL:    u.photoURL || null,
            anonEmoji:   u.anonEmoji || null,
            anonColor:   u.anonColor || null,
            status:      u.status || 'offline',
          };
        });
      });
      Promise.all(promises).then(function(convs) {
        convs = convs.filter(Boolean);
        if (!convs.length) {
          list.innerHTML = '<div class="empty-state">' + SVG.chat + '<span>Aucune conversation</span></div>';
          return;
        }
        S._dmConvs = convs; // cache pour filtrage
        renderDmConvList(convs);
      });
    }, function() {
      list.innerHTML = '<div class="empty-state">' + SVG.chat + '<span>Erreur chargement</span></div>';
    });
}

function renderDmConvList(convs) {
  var list = document.getElementById('dmConvList');
  if (!list) return;
  list.innerHTML = convs.map(function(c) {
    var statusDot = { online:'#10b981', away:'#f59e0b', dnd:'#ef4444', offline:'#6b7280' }[c.status] || '#6b7280';
    var avContent = c.photoURL
      ? '<img src="' + c.photoURL + '" style="width:100%;height:100%;object-fit:cover;border-radius:50%"/>'
      : (c.anonEmoji || c.name[0].toUpperCase());
    var avStyle = c.anonColor ? 'background:' + c.anonColor : '';
    return '<div class="room-row" onclick="openDmConv(\'' + c.dmId + '\',\'' + c.otherId + '\',\'' + esc(c.name) + '\',\'' + (c.photoURL || '') + '\')">' +
      '<div class="room-row-icon" style="border-radius:50%;' + avStyle + '">' + avContent + '</div>' +
      '<div class="room-row-info">' +
        '<div class="room-row-name">' + esc(c.name) + '</div>' +
        '<div class="room-row-last">' + esc(c.lastMessage || 'Aucun message') + '</div>' +
      '</div>' +
...
function filterDmList(q) {
  if (!S._dmConvs) return;
  var filtered = q
    ? S._dmConvs.filter(function(c) { return c.name.toLowerCase().includes(q.toLowerCase()); })
    : S._dmConvs;
  renderDmConvList(filtered);
}

// Ouvrir une conversation DM depuis la liste (vue Mes Messages)
function openDmConv(dmId, otherId, otherName, otherPhoto) {
  S.activeDmId    = dmId;
  S.activeDmTarget = { uid: otherId, name: otherName, photo: otherPhoto };

  // S'assurer que le document DM existe
  db.collection('dms').doc(dmId).set({
    participants: [S.user.uid, otherId],
    updatedAt: firebase.firestore.FieldValue.serverTimestamp(),
  }, { merge: true });

  // Afficher la zone de chat DM dans viewDms
  var empty  = document.getElementById('dmEmpty');
  var active = document.getElementById('dmChatActive');
  if (empty)  empty.style.display  = 'none';
  if (active) active.style.display = 'flex';

  // Header
  var avEl   = document.getElementById('dmChatAvatar');
  var nameEl = document.getElementById('dmChatName');
  var statEl = document.getElementById('dmChatStatus');
  if (avEl) {
    if (otherPhoto) {
      avEl.innerHTML = '<img src="' + otherPhoto + '" style="width:100%;height:100%;object-fit:cover;border-radius:50%"/>';
      avEl.style.background = 'none';
    } else {
      avEl.textContent = (otherName || 'U')[0].toUpperCase();
      avEl.style.background = 'var(--grad)';
    }
  }
  if (nameEl) nameEl.textContent = otherName || 'Contact';
  if (statEl) statEl.textContent = '● En ligne';

  // Bouton retour sur mobile
  var backBtn = document.getElementById('dmBackBtn');
  if (backBtn) backBtn.style.display = window.innerWidth <= 768 ? 'flex' : 'none';

  // Charger les messages dans dmMessages (panel flottant partagé)
  loadDmMessages(dmId);

  // Réattacher dmMessages au bon conteneur (dmChatActive plutôt que dmPanel)
  // On crée un nouveau conteneur dédié pour la vue DM inline
  var dmChatMsgsId = 'dmInlineMsgs';
  var existing = document.getElementById(dmChatMsgsId);
  if (!existing) {
    existing = document.createElement('div');
    existing.id = dmChatMsgsId;
    existing.className = 'messages-scroll';
    existing.style.cssText = 'flex:1;overflow-y:auto;padding:12px;display:flex;flex-direction:column;gap:4px';
    // Zone de saisie inline
    var inputZone = document.createElement('div');
    inputZone.className = 'input-area';
    inputZone.innerHTML =
      '<div class="msg-input-wrap">' +
        '<div class="msg-box" id="dmInlineBox" contenteditable="true" data-placeholder="Message..." onkeydown="onDmInlineKey(event)"></div>' +
      '</div>' +
      '<button class="send-btn" onclick="sendDmInline()">' +
        '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>' +
      '</button>';
    active.appendChild(existing);
    active.appendChild(inputZone);
  }

  // Écouter les messages dans la vue inline
  if (S.dmInlineUnsub) S.dmInlineUnsub();
  S.dmInlineUnsub = db.collection('dms').doc(dmId).collection('messages')
    .orderBy('createdAt', 'asc').limit(100)
    .onSnapshot(function(snap) {
      var msgs = snap.docs.map(function(d) { return Object.assign({ id: d.id }, d.data()); });
      existing.innerHTML = msgs.map(function(m) {
        var isOwn = m.uid === S.user.uid;
        var time  = m.createdAt && m.createdAt.toMillis ? new Date(m.createdAt.toMillis()).toLocaleTimeString('fr-FR', { hour:'2-digit', minute:'2-digit' }) : '';
        return '<div class="msg-row' + (isOwn ? ' own' : '') + '">' +
          '<div class="msg-content">' +
...
function closeDmChat() {
  ...
  if (S.dmInlineUnsub) { S.dmInlineUnsub(); S.dmInlineUnsub = null; }
  S.activeDmId = null;
}

function sendDmInline() {
  if (!S.user || !S.activeDmId) return;
  var box = document.getElementById('dmInlineBox');
...
function onDmInlineKey(e) {
  if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendDmInline(); }
}

// ─────────────────────────────────────────────
//  BADGE DM — nav mobile + desktop
// ─────────────────────────────────────────────
var dmBadgeUnsub = null;

function listenDmBadge() {
  if (!S.user) return;
  if (dmBadgeUnsub) dmBadgeUnsub();

  dmBadgeUnsub = db.collection('dms')
    .where('participants', 'array-contains', S.user.uid)
    .onSnapshot(function(snap) {
      // Compter les DMs avec au moins un message non lu
      var unreadCount = 0;
      var promises = snap.docs.map(function(d) {
        return d.ref.collection('messages')
          .where('readBy', 'array-contains', S.user.uid)
          .get().then(function(readSnap) {
            return null; // on ne se base pas sur ça, trop coûteux
          }).catch(function() { return null; });
      });

      // Approche simple : compter les convs avec lastMessage non lu
      // On vérifie si le dernier message n'est pas de nous et n'est pas lu par nous
      var count = 0;
      snap.docs.forEach(function(d) {
        var data = d.data();
        // Si lastMessageUid est défini et différent de nous → potentiellement non lu
        if (data.lastMessageUid && data.lastMessageUid !== S.user.uid && !data.readByMe) {
          count++;
        }
      });
...
function updateDmBadge(count) {
  // Mobile nav
  var mbnBadge = document.getElementById('mbnMsgBadge');
  if (mbnBadge) {
    if (count > 0) { mbnBadge.textContent = count > 9 ? '9+' : count; mbnBadge.style.display = 'flex'; }
    else mbnBadge.style.display = 'none';
  }
  // Desktop nav
  var navBadge = document.getElementById('dmNavBadge');
  if (navBadge) {
    if (count > 0) { navBadge.textContent = count > 9 ? '9+' : count; navBadge.style.display = 'flex'; }
    else navBadge.style.display = 'none';
  }
}

// ─────────────────────────────────────────────
//  ALIASES — setEphem (appelé depuis modal HTML)
// ─────────────────────────────────────────────
function setEphem(hours) {
  S.ephemPick = hours;
  S.ephemHours = hours;
  localStorage.setItem('sis_ephem', hours);
  updateEphemBadges();
  closeOverlay('ephemOverlay');
  toast('Messages éphémères : ' + (hours ? hours + 'h' : 'Off'), 'success');
}

// ─────────────────────────────────────────────
//  ALIAS — setCustomBg (appelé depuis panel thème)
// ─────────────────────────────────────────────
...
Object.assign(window, {
  mobileNav, loadDmList, filterDmList, openDmConv, closeDmChat,
  sendDmInline, onDmInlineKey, setEphem, setCustomBg,
  listenDmBadge, updateDmBadge,
});
