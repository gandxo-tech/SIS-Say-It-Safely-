...
(function () {
  ...
  (function injectBotStyles() {
    ...
    s.textContent = [
      ...
      /* manga / webtoon */
      '.bc-anime-jp{font-size:10px;color:var(--text-muted,#5C5C80);font-style:italic;margin-bottom:4px}',
      '.bc-anime-subtitle{font-size:11px;color:var(--text-secondary,#9898C0);margin-bottom:4px}',
      '.bc-anime-meta-row{display:flex;flex-wrap:wrap;gap:4px;margin-bottom:6px}',
      '.bc-anime-layout{display:flex;gap:12px;align-items:flex-start;margin-bottom:10px}',
      '.bc-anime-poster{flex-shrink:0;width:90px}',
      '.bc-tags-row{display:flex;flex-wrap:wrap;gap:3px;margin-bottom:6px}',
      '.bc-score-row{display:flex;align-items:center;gap:6px;margin-bottom:6px}',
      '.bc-score-num{font-size:13px;font-weight:700;color:#F59E0B}',
      '.bc-score-sub{font-size:10px;color:var(--text-muted,#5C5C80)}',
      '.bc-synopsis{font-size:11.5px;line-height:1.6;color:rgba(255,255,255,0.72);margin-bottom:10px;background:rgba(255,255,255,0.04);border-radius:8px;padding:8px 10px;border-left:2px solid rgba(91,142,244,0.4)}',
      ...
      '.bc-stats{display:flex;gap:8px;flex-wrap:wrap}',
      '.bc-stats span{font-size:11px;color:var(--text-muted,#5C5C80)}',
      '.bc-link{font-size:11px;color:#5B8EF4;text-decoration:none}',
      ...
      /* lyrics */
      '.bc-lyrics-header{display:flex;align-items:center;gap:12px;margin-bottom:12px}',
      '.bc-lyrics-body{font-size:12px;line-height:1.85;color:rgba(255,255,255,0.82);background:rgba(255,255,255,0.04);border-radius:10px;padding:12px 14px;max-height:340px;overflow-y:auto;border-left:2px solid #EC4899;white-space:pre-line}',
      /* gif neko / waifu */
  ...
  function handleBotCommand(text) {
    ...
    /* ─ Paroles ─ */
      ...
      if (!args) {
        ...
        showToast('Usage : /paroles Artiste - Titre', 'warning');
        return;
      }
    ...
    /* ─ Aide ─ */
      ...
      sendMessage({
        type: 'text',
        text: '🤖 Commandes SIS Bot\n\n🎞 /anime [titre] · /manga [titre] · /webtoon · /manhwa · /manhua\n🎮 /pokemon [nom] · 🎬 /film [titre]\n🤗 /hug · /pat · /kiss · /cry · /wave · /waifu\n💬 /blague · /citation [thème] · /chuck · /fact\n🎵 /paroles [artiste - titre]\n🌍 /pays [pays] · /recette [plat] · /meme\n\n→ Tape / pour voir les suggestions'
      });
      return;
    }
    else {
  ...
  var BOT_CMDS = [
    { cmd: '/anime',    args: '[titre]',  desc: 'Fiche complète animé 🎞' },
    { cmd: '/manga',    args: '[titre]',  desc: 'Fiche manga Jikan 📖' },
    ...
    { cmd: '/blague',   args: '',         desc: 'Blague en français 😂' },
    { cmd: '/citation', args: '[thème]',  desc: 'Citation inspirante 💬' },
    { cmd: '/pays',     args: '[pays]',   desc: 'Infos sur un pays 🌍' },
    ...
    { cmd: '/aide',     args: '',         desc: 'Toutes les commandes du bot' }
  ];
  ...
  function bcFetchManga(q) {
    return fetch('https://api.jikan.moe/v4/manga?q=' + encodeURIComponent(q) + '&limit=1')
      .then(function(r) { if (!r.ok) throw new Error('Jikan erreur ' + r.status); return r.json(); })
      .then(function(d) {
        if (!d.data || !d.data[0]) throw new Error('Manga introuvable : ' + q);
        var a = d.data[0];
        var GENRE_FR = {
          'Action':'Action','Adventure':'Aventure','Comedy':'Comédie','Drama':'Drame',
          'Fantasy':'Fantaisie','Horror':'Horreur','Mystery':'Mystère','Romance':'Romance',
          'Sci-Fi':'Sci-Fi','Sports':'Sport','Supernatural':'Surnaturel',
          'Slice of Life':'Tranche de vie','School':'Vie scolaire','Historical':'Historique',
          'Military':'Militaire','Psychological':'Psychologique','Martial Arts':'Arts martiaux',
          'Thriller':'Thriller','Isekai':'Isekai','Magic':'Magie','Vampire':'Vampire',
          'Mecha':'Mecha','Shounen':'Shōnen','Shoujo':'Shōjo','Seinen':'Seinen','Josei':'Josei'
        };
        var STATUS_FR = {
          'Finished':'✅ Terminé','Publishing':'🔴 En cours',
          'On Hiatus':'⏸ Pause','Discontinued':'🚫 Arrêté','Not yet published':'🟡 À venir'
        };
        var synopsis = (a.synopsis || '')
          .replace(/\[Written.*?\]/g, '').replace(/\(Source.*?\)/gi, '').trim();
        if (synopsis.length > 220) synopsis = synopsis.substring(0, 220) + '…';
        return {
          cardType:      'manga',
          label:         '📖 ' + (a.title_french || a.title_english || a.title),
          ...
          synopsis:      synopsis,
          genres:        (a.genres || []).concat(a.themes || []).slice(0, 4)
                           .map(function(g){ return GENRE_FR[g.name] || g.name; }),
          authors:       (a.authors || []).slice(0, 2)
                           .map(function(au){ return au.name.split(', ').reverse().join(' '); }),
          demographics:  (a.demographics || []).slice(0, 1)
                           .map(function(d){ return GENRE_FR[d.name] || d.name; }),
          type:          a.type || 'Manga',
          url:           a.url,
  ...
  function bcFetchWebtoon(q) {
    return fetch('https://api.mangadex.org/manga?title='+encodeURIComponent(q)+'&limit=5&contentRating[]=safe&contentRating[]=suggestive&includes[]=cover_art&order[relevance]=desc')
      .then(function(r){ return r.json(); })
      .then(function(d) {
        if (!d.data || !d.data.length) throw new Error('Webtoon introuvable : ' + q);
        var chosen = d.data[0];
        for (var i = 0; i < d.data.length; i++) {
          var langs = d.data[i].attributes.availableTranslatedLanguages || [];
          if (langs.indexOf('fr') !== -1) { chosen = d.data[i]; break; }
        }
        return { data: [chosen] };
      })
      .then(function(d) {
  ...
  function bcFetchCitation(theme) {
    /* quotable.io est hors ligne — on utilise zenquotes.io (gratuit, sans clé) */
    /* + fallback sur une liste FR si réseau KO                                 */
    var QUOTES_FR = [
      ...
      { text: "Le bonheur n'est pas quelque chose que l'on reporte à demain ; il se cultive dans le présent.", author: "Jim Rohn" },
      { text: "Dans le milieu du chemin de notre vie, je me retrouvai dans une forêt obscure.", author: "Dante Alighieri" },
      { text: "Mieux vaut être seul que mal accompagné.", author: "George Washington" },
      { text: "Chaque jour est une nouvelle chance de changer ta vie.", author: "Anonyme" },
      { text: "Les rêves ne deviennent réalité que si l'on travaille.", author: "Anonyme" },
    ...
    return fetch('https://zenquotes.io/api/random')
      ...
      .then(function(d) {
        /* zenquotes répond avec un tableau [{q, a}] */
        var item = Array.isArray(d) ? d[0] : d;
        if (!item || !item.q) throw new Error('Format inattendu');
        /* Filtrer par thème côté client si demandé */
        return {
          cardType: 'citation',
          label:    '💬 Citation',
          text:     item.q,
          author:   item.a || 'Inconnu',
          tags:     theme ? [theme] : [],
          lang:     'en'
        };
      })
      .catch(function() {
        /* Fallback — liste FR hardcodée */
        var q = QUOTES_FR[Math.floor(Math.random() * QUOTES_FR.length)];
        if (theme) {
          /* Chercher une citation contenant le thème */
          var filtered = QUOTES_FR.filter(function(x){ return x.text.toLowerCase().indexOf(theme.toLowerCase()) !== -1 || x.author.toLowerCase().indexOf(theme.toLowerCase()) !== -1; });
          if (filtered.length) q = filtered[Math.floor(Math.random() * filtered.length)];
        }
        return {
          cardType: 'citation',
          label:    '💬 Citation',
  ...
  /* ── Paroles — lyrics.ovh (sans clé) ── */
  function bcFetchLyrics(q) {
    var artist = '', title = '';
    /* Accepte "Artiste - Titre" ou "Artiste Titre" */
    var dash = q.indexOf(' - ');
    if (dash !== -1) {
      artist = q.substring(0, dash).trim();
      title  = q.substring(dash + 3).trim();
    } else {
      var parts = q.trim().split(/\s+/);
      artist = parts[0];
      title  = parts.slice(1).join(' ');
    }
    if (!title) throw new Error('Format : /paroles Artiste - Titre');
    ...
    return fetch(primary)
      ...
      .then(function(d) {
        if (!d.lyrics) throw new Error('Pas de paroles');
        return d.lyrics;
      })
      .catch(function() {
        ...
        return fetch(fallback)
          .then(function(r2) { return r2.json(); })
          .then(function(d2) {
            if (!d2.lyrics) throw new Error('Paroles introuvables pour : ' + q);
            return d2.lyrics;
          });
      })
  ...
  function buildBotCardHtml(data) {
    ...
    if (ct === 'manga') {
      ...
      return '<div class="bot-card">'+
        '<div class="bc-stripe" style="background:linear-gradient(90deg,#10B981,#3B82F6)"></div>'+
        '<div class="bc-body">'+
          cmdLine+
          '<div class="bc-anime-layout">'+
            (imgHtml ? '<div class="bc-anime-poster">'+imgHtml+'</div>' : '')+
            '<div class="bc-anime-info">'+
              '<div class="bc-anime-title">'+escapeHtml(cd.title||'')+'</div>'+
              (cd.titleOriginal && cd.title!==cd.titleOriginal ? '<div class="bc-anime-subtitle">'+escapeHtml(cd.titleOriginal)+'</div>' : '')+
              (cd.titleJp ? '<div class="bc-anime-jp">'+escapeHtml(cd.titleJp)+'</div>' : '')+
              '<div class="bc-anime-meta-row">'+
              ...
              (cd.authors&&cd.authors.length ? '<div class="bc-anime-subtitle" style="margin-top:4px">✍️ '+escapeHtml(cd.authors.join(', '))+'</div>' : '')+
            '</div>'+
          '</div>'+
          (cd.synopsis ? '<div class="bc-synopsis">'+escapeHtml(cd.synopsis)+'</div>' : '')+
          '<div class="bc-footer">'+
            '<div class="bc-stats">'+
              ...
              (cd.rank     ? '<span>🏆 #'+cd.rank+'</span>' : '')+
            '</div>'+
            (cd.url ? '<a class="bc-link" href="'+escapeHtml(cd.url)+'" target="_blank" rel="noopener">MAL →</a>' : '')+
          '</div>'+
        '</div>'+
      '</div>';
    }
      ...
      var langBadgesHtml = (cd.langBadges||[]).map(function(l){ return '<span class="bc-badge" style="font-size:11px">'+escapeHtml(l)+'</span>'; }).join('');
      return '<div class="bot-card">'+
        '<div class="bc-stripe" style="background:linear-gradient(90deg,#EC4899,#8B5CF6)"></div>'+
        '<div class="bc-body">'+
          cmdLine+
          '<div class="bc-anime-layout">'+
            (imgHtml ? '<div class="bc-anime-poster">'+imgHtml+'</div>' : '')+
            '<div class="bc-anime-info">'+
              ...
              '</div>'+
              (cd.authors&&cd.authors.length ? '<div class="bc-anime-subtitle" style="margin-top:4px">✍️ '+escapeHtml(cd.authors.join(', '))+'</div>' : '')+
              '<div class="bc-tags-row">'+tags+langBadgesHtml+'</div>'+
            '</div>'+
          '</div>'+
          (cd.synopsis ? '<div class="bc-synopsis">'+escapeHtml(cd.synopsis)+'</div>' : '')+
          '<div class="bc-footer">'+
            ...
            '</div>'+
            (cd.url ? '<a class="bc-link" href="'+escapeHtml(cd.url)+'" target="_blank" rel="noopener">MangaDex →</a>' : '')+
          '</div>'+
        '</div>'+
      '</div>';
    }
    ...
    if (ct === 'pays') {
      ...
      '</div></div>';
        ...
        if (ct === 'lyrics') {
      var linesHtml = escapeHtml(cd.lyrics||'').replace(/\n/g,'<br>');
      return '<div class="bot-card">'+
        '<div class="bc-stripe" style="background:linear-gradient(90deg,#EC4899,#F97316)"></div>'+
        '<div class="bc-body">'+
          cmdLine+
          '<div class="bc-lyrics-header">'+
            '<div style="font-size:28px;line-height:1">🎵</div>'+
            '<div>'+
              '<div class="bc-anime-title">'+escapeHtml(cd.title||'')+'</div>'+
              '<div class="bc-anime-subtitle">'+escapeHtml(cd.artist||'')+'</div>'+
            '</div>'+
          '</div>'+
          '<div class="bc-lyrics-body">'+linesHtml+'</div>'+
        '</div>'+
      '</div>';
    }
