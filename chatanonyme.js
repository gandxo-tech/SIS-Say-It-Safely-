...
(function () {
  ...
  function appendMessage(msgId, data, compact) {
    ...
    /* Clic sur bulle de réponse → scroll vers le message original */
    var replyBubble = wrap.querySelector('.bubble-reply[data-goto-msg]');
    if (replyBubble) {
      replyBubble.addEventListener('click', function(e) {
        var targetId = replyBubble.dataset.gotoMsg;
        if (!targetId) return;
  ...
  function buildMessageHtml(msgId, data, isOut, compact) {
    ...
    var replyHtml = '';
    if (data.replyTo) {
      var _replyIcon = {image:'🖼️ ',voice:'🎤 ',gif:'🎬 ',poll:'📊 ',sticker:'📋 ','bot-card':'🤖 '};
      var _replyPreview = data.replyTo.text ? data.replyTo.text.substr(0,80) : (_replyIcon[data.replyTo.type]||'📎 ')+' Média';
      replyHtml = '<div class="bubble-reply" data-goto-msg="'+escapeHtml(data.replyTo.msgId||'')+'"><div class="bubble-reply-author">'+escapeHtml(data.replyTo.authorPseudo||'')+'</div><div class="bubble-reply-text">'+escapeHtml(_replyPreview)+'</div></div>';
    }
  ...
  function buildVoiceHtml(msgId, data) {
    ...
    var bars = '';
    var barCount = 20;
    /* Barres déterministes basées sur l'ID du message */
    var seed = 0;
    for (var s = 0; s < msgId.length; s++) seed = (seed * 31 + msgId.charCodeAt(s)) & 0xFFFFFF;
  ...
  function initInputBar() {
    ...
    if (btnImg && fileImg) {
      ...
      fileImg.addEventListener('change', function(e) {
        var file = e.target.files[0];
        if (!file) return;
        if (file.size > 20*1024*1024) { showToast('Image trop lourde (max 20 Mo)','error'); return; }
        hide(attachMenu);
        compressImage(file, 1200, 0.82, function(blob) {
          uploadToCloudinary(blob || file, function(url) {
  ...
  function setReply(msgId, data) {
    ...
    var TYPE_LABEL = {'image':'🖼️ Photo','voice':'🎤 Message vocal','gif':'🎬 GIF','sticker':'📋 Sticker','poll':'📊 Sondage','bot-card':'🤖 Commande bot'};
    if (msg) msg.textContent = data.text ? data.text.substr(0,60) : (TYPE_LABEL[data.type] || '📎 Média');
    show(bar);
    var input = $('msgInput');
  ...
  function startEditMessage(msgId, data) {
    ...
    /* Barre visuelle édition */
    var editBar = document.getElementById('sis-edit-bar');
    if (!editBar) {
      editBar = document.createElement('div');
      editBar.id = 'sis-edit-bar';
      editBar.style.cssText = 'display:flex;align-items:center;justify-content:space-between;padding:6px 14px;background:rgba(91,142,244,0.12);border-top:1px solid rgba(91,142,244,0.3);font-size:12px;color:#5B8EF4;';
      editBar.innerHTML = '<span>✏️ Modification en cours…</span><button style="background:none;border:none;color:#EF4444;font-size:12px;cursor:pointer;padding:2px 6px;">Annuler (Échap)</button>';
    ...
    if (btnSend) {
      ...
      var cancelBtn = editBar.querySelector('button');
      ...
      btnSend.onclick = function() {
        ...
        db.collection('rooms').doc(currentRoomId).collection('messages').doc(msgId)
          ...
          .then(function(){ showToast('Message modifié ✓','success',1500); }).catch(function(){});
        cancelEdit();
      };
    }
  }
  ...
  function compressImage(file, maxSize, quality, cb) {
    if (!file.type.startsWith('image/') || file.type === 'image/gif') { cb(null); return; }
    var reader = new FileReader();
    reader.onload = function(ev) {
      var img = new Image();
      img.onload = function() {
        var w = img.width, h = img.height;
        if (w <= maxSize && h <= maxSize && file.size < 500*1024) { cb(null); return; }
  ...
  function applyChatBackground(type, value, roomId) {
    ...
    if (chatArea) chatArea.classList.toggle('has-custom-bg', !(!type || type === 'default'));
  ...
  var _randomCurrentPartnerData = null;

  function showRandomChat(partnerId, partnerData, sessionId) {
    _randomCurrentPartnerData = partnerData;
    hide($('randomSearching'));
    ...
    /* Bouton DM invite */
    var btnDm = $('btnDmInviteRandom');
    if (btnDm) {
      btnDm.style.display = '';
      btnDm.onclick = function() { sendDmInvite(partnerId, partnerData, sessionId); };
    }

    /* Écouter déconnexion partenaire */
    var sessionRef = db.collection('randomSessions').doc(sessionId);
    if (randomListenerOff) { randomListenerOff(); randomListenerOff = null; }

    /* Listener session (active:false = partenaire parti) */
    var _sessionListenerOff = sessionRef.onSnapshot(function(snap) {
      if (!snap.exists) return;
      var d = snap.data();
      if (!d.active && randomSessionId === sessionId) {
        _sessionListenerOff();
        showToast('Ton partenaire a quitté la conversation 👋', 'info', 4000);
        var btn = $('btnSkipRandom');
        if (btn) btn.style.background = 'rgba(91,142,244,0.3)';
      }
    }, function(){});

    /* Listener messages — incrémental (pas de innerHTML = '') */
    var _msgRenderedIds = {};
    var _msgsListenerOff = sessionRef.collection('messages')
      ...
      .onSnapshot(function(snap) {
        ...
        snap.docChanges().forEach(function(change) {
          ...
          var doc = change.doc;
          if (_msgRenderedIds[doc.id]) return;
          _msgRenderedIds[doc.id] = true;
          var d = doc.data();
          var isOut = d.senderId === currentUser.uid;
          /* Typing indicator — effacer quand message arrive du partenaire */
          if (!isOut) {
            var typi = $('randomTypingIndicator');
            if (typi) typi.style.display = 'none';
          }
          var wrap = document.createElement('div');
          wrap.className = 'msg-wrap ' + (isOut ? 'out' : 'in');
          var timeStr = fmtTime(d.createdAt);
          if (d.isReaction) {
            wrap.innerHTML = '<div class="rnd-reaction-bubble">' + escapeHtml(d.text||'') + '</div>';
          } else {
            wrap.innerHTML =
              '<div class="msg-row">' +
    ...
    /* Combiner les deux listeners dans randomListenerOff */
    randomListenerOff = function() {
      _sessionListenerOff();
      _msgsListenerOff();
    };

    /* Typing indicator — émettre quand on tape */
    var randInput = $('randomMsgInput');
    if (randInput) {
      var _rTypingTimer = null;
      randInput.addEventListener('input', function() {
        sessionRef.update({ typingUid: currentUser.uid }).catch(function(){});
        if (_rTypingTimer) clearTimeout(_rTypingTimer);
        _rTypingTimer = setTimeout(function(){
          sessionRef.update({ typingUid: '' }).catch(function(){});
        }, 2000);
      });
    }

    /* Écouter le typing de l'autre */
    var _typingListener = sessionRef.onSnapshot(function(snap) {
      if (!snap.exists) return;
      var d = snap.data();
      var typi = $('randomTypingIndicator');
      if (!typi) return;
      if (d.typingUid && d.typingUid !== currentUser.uid) {
        typi.style.display = 'flex';
      } else {
        typi.style.display = 'none';
      }
    }, function(){});

    /* Écouter invitation DM reçue */
    var _dmInviteListener = sessionRef.onSnapshot(function(snap) {
      if (!snap.exists) return;
      var d = snap.data();
      if (d.dmInvite && d.dmInvite.to === currentUser.uid && d.dmInvite.status === 'pending') {
        showDmInviteReceived(d.dmInvite.from, d.dmInvite.fromData, sessionId);
      }
    }, function(){});
  }

  /* ── Envoyer invitation DM ── */
  function sendDmInvite(partnerId, partnerData, sessionId) {
    if (!sessionId) return;
    db.collection('randomSessions').doc(sessionId).update({
      dmInvite: {
        from:     currentUser.uid,
        fromData: { pseudo: currentUserData.pseudo||'Anonyme', emoji: currentUserData.emoji||'', color: currentUserData.color||'#5B8EF4' },
        to:       partnerId,
        status:   'pending'
      }
    }).catch(function(){});
    showToast('Invitation DM envoyée 💌', 'success');
    var btn = $('btnDmInviteRandom');
    if (btn) { btn.textContent = '⏳ En attente…'; btn.disabled = true; }
  }

  /* ── Afficher banner invitation DM reçue ── */
  function showDmInviteReceived(fromUid, fromData, sessionId) {
    var banner = $('randomDmInviteBanner');
    if (!banner || banner.dataset.shown === '1') return;
    banner.dataset.shown = '1';
    banner.style.display = '';
    var fromName = (fromData && fromData.pseudo) ? fromData.pseudo : 'Ton partenaire';
    var txtEl = $('randomDmInviteText');
    if (txtEl) txtEl.textContent = fromName + ' veut continuer en DM 💌';
    var btnAccept = $('btnDmInviteAccept');
    var btnDecline = $('btnDmInviteDecline');
    if (btnAccept) {
      btnAccept.onclick = function() {
        db.collection('randomSessions').doc(sessionId).update({ 'dmInvite.status': 'accepted' }).catch(function(){});
        banner.style.display = 'none';
        openDm(fromUid, fromData || {});
        leaveRandom();
        showToast('Conversation DM ouverte ! 🎉', 'success');
      };
    }
    if (btnDecline) {
      btnDecline.onclick = function() {
        db.collection('randomSessions').doc(sessionId).update({ 'dmInvite.status': 'declined' }).catch(function(){});
        banner.style.display = 'none';
  ...
  function leaveRandom() {
    ...
    _randomCurrentPartnerData = null;
    /* Cacher DM invite banner */
    var banner = $('randomDmInviteBanner');
    if (banner) { banner.style.display = 'none'; banner.dataset.shown = '0'; }
    var btnDm = $('btnDmInviteRandom');
    if (btnDm) { btnDm.style.display = 'none'; btnDm.disabled = false; btnDm.textContent = '💌 Inviter en DM'; }
  ...
  function runGlobalSearch(q) {
    ...
    /* Recherche insensible à la casse via champ lowercase */
    var qL = q.toLowerCase();
    var roomsPromise = db.collection('rooms').orderBy('nameLower').startAt(qL).endAt(qL+'\uf8ff').limit(5).get()
      .catch(function(){ return db.collection('rooms').orderBy('name').startAt(q).endAt(q+'\uf8ff').limit(5).get(); });
    var usersPromise = db.collection('users').orderBy('pseudoLower').startAt(qL).endAt(qL+'\uf8ff').limit(5).get()
      .catch(function(){ return db.collection('users').orderBy('pseudo').startAt(q).endAt(q+'\uf8ff').limit(5).get(); });
  ...
  /* ── Injection CSS random chat enrichi ── */
  (function injectRandomStyles() {
    if (document.getElementById('sis-random-styles')) return;
    var s = document.createElement('style');
    s.id = 'sis-random-styles';
    s.textContent = [
      /* Typing indicator */
      '#randomTypingIndicator{display:none;align-items:center;gap:6px;padding:4px 10px;font-size:11px;color:var(--text-muted,#5C5C80)}',
      '#randomTypingIndicator .typing-dot{width:5px;height:5px;border-radius:50%;background:#5B8EF4;animation:typingBounce 1.2s infinite}',
      '#randomTypingIndicator .typing-dot:nth-child(2){animation-delay:.2s}',
      '#randomTypingIndicator .typing-dot:nth-child(3){animation-delay:.4s}',
      '@keyframes typingBounce{0%,60%,100%{transform:translateY(0)}30%{transform:translateY(-4px)}}',
      /* DM invite banner */
      '#randomDmInviteBanner{display:none;align-items:center;gap:8px;padding:10px 14px;background:linear-gradient(90deg,rgba(139,92,246,0.18),rgba(91,142,244,0.12));border:1px solid rgba(139,92,246,0.35);border-radius:12px;margin:6px 10px}',
      '#randomDmInviteText{flex:1;font-size:13px;font-weight:600;color:var(--text-primary,#F0F0FA)}',
      '#btnDmInviteAccept{padding:6px 14px;border-radius:20px;border:none;background:linear-gradient(90deg,#8B5CF6,#5B8EF4);color:#fff;font-size:12px;font-weight:700;cursor:pointer}',
      '#btnDmInviteDecline{padding:6px 10px;border-radius:20px;border:1px solid rgba(255,255,255,0.15);background:transparent;color:rgba(255,255,255,0.5);font-size:12px;cursor:pointer}',
      /* Bouton DM */
      '#btnDmInviteRandom{display:none;padding:6px 14px;border-radius:20px;border:1px solid rgba(139,92,246,0.45);background:rgba(139,92,246,0.12);color:#8B5CF6;font-size:12px;font-weight:600;cursor:pointer;transition:all .2s}',
      '#btnDmInviteRandom:hover{background:rgba(139,92,246,0.25)}',
      /* Réaction rapide */
      '.rnd-reaction-bubble{text-align:center;font-size:22px;padding:2px 0;line-height:1.4}'
    ].join('');
    document.head.appendChild(s);

    /* Injecter le DOM manquant dans randomChatActive si les éléments n'existent pas */
    setTimeout(function() {
      var chatActive = $('randomChatActive');
      if (!chatActive) return;
      if (!$('randomTypingIndicator')) {
        var typi = document.createElement('div');
        typi.id = 'randomTypingIndicator';
        typi.innerHTML = '<span class="typing-dot"></span><span class="typing-dot"></span><span class="typing-dot"></span><span style="margin-left:4px">tape…</span>';
        var msgs = $('randomMessages');
        if (msgs && msgs.parentNode) msgs.parentNode.insertBefore(typi, msgs.nextSibling);
      }
      if (!$('randomDmInviteBanner')) {
        var banner = document.createElement('div');
        banner.id = 'randomDmInviteBanner';
        banner.innerHTML = '<span id="randomDmInviteText">Ton partenaire veut continuer en DM 💌</span><button id="btnDmInviteAccept">✓ Accepter</button><button id="btnDmInviteDecline">✕</button>';
        chatActive.insertBefore(banner, chatActive.firstChild);
      }
      if (!$('btnDmInviteRandom')) {
        /* Chercher la barre d'actions (btnSkipRandom) */
        var skip = $('btnSkipRandom');
        if (skip && skip.parentNode) {
          var dmBtn = document.createElement('button');
          dmBtn.id = 'btnDmInviteRandom';
          dmBtn.textContent = '💌 Inviter en DM';
          skip.parentNode.insertBefore(dmBtn, skip);
        }
      }
    }, 500);
  })();
  ...
  (function injectBotStyles() {
    ...
    s.textContent = [
      ...
      '.bc-anime-sub{font-size:11px;color:var(--text-secondary,#9898C0);margin-bottom:4px}',
      '.bc-badge{display:inline-block;font-size:10px;padding:2px 7px;border-radius:20px;background:rgba(91,142,244,0.18);color:#5B8EF4;border:1px solid rgba(91,142,244,0.3)}',
      '.bc-badge-type{background:rgba(139,92,246,0.18);color:#8B5CF6;border-color:rgba(139,92,246,0.3)}',
      '.bc-badge-dim{background:rgba(255,255,255,0.06);color:rgba(255,255,255,0.45);border-color:rgba(255,255,255,0.1)}',
      /* gif neko / waifu */
      ...
      /* spoiler */
      '.spoiler{background:rgba(255,255,255,0.12);color:transparent;border-radius:4px;cursor:pointer;padding:0 3px;user-select:none;transition:all .2s}',
      '.spoiler.revealed{background:rgba(255,255,255,0.08);color:inherit}',
      /* message highlight (scroll-to) */
      '.msg-highlight .bubble{animation:msgFlash .4s ease 3}',
      '@keyframes msgFlash{0%,100%{opacity:1}50%{opacity:0.35;transform:scale(1.01)}}',
      /* bulle réponse cliquable */
      '.bubble-reply[data-goto-msg]{cursor:pointer;transition:background .15s}',
      '.bubble-reply[data-goto-msg]:hover{background:rgba(91,142,244,0.15)}',
      /* bulles adaptatives au fond */
      '.has-custom-bg .bubble{backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px)}',
      '.has-custom-bg .msg-wrap.out .bubble{background:rgba(29,78,216,0.82) !important}',
      '.has-custom-bg .msg-wrap.in .bubble{background:rgba(20,20,40,0.78) !important;border-color:rgba(255,255,255,0.12) !important}',
      '.has-custom-bg .msg-wrap.out .bubble::after{border-left-color:rgba(29,78,216,0.82) !important}',
      '.has-custom-bg .msg-wrap.in .bubble::before{border-right-color:rgba(20,20,40,0.78) !important}',
      /* carte anime enrichie */
      '.bc-card-anime .bc-body{padding:14px 14px 10px}',
      '.bc-anime-layout{display:flex;gap:12px;align-items:flex-start;margin-bottom:10px}',
      '.bc-anime-poster{flex-shrink:0;width:90px}',
      '.bc-anime-cover{width:90px;height:130px;object-fit:cover;border-radius:8px;display:block}',
      '.bc-anime-info{flex:1;min-width:0}',
      '.bc-anime-title{font-size:14px;font-weight:700;line-height:1.3;margin-bottom:3px}',
      '.bc-anime-subtitle{font-size:11px;color:rgba(255,255,255,0.5);margin-bottom:4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}',
      '.bc-anime-jp{font-size:10px;color:rgba(255,255,255,0.35);margin-bottom:6px}',
      '.bc-anime-meta-row{display:flex;gap:4px;flex-wrap:wrap;margin-bottom:6px}',
      '.bc-badge{font-size:10px;padding:2px 7px;border-radius:20px;background:rgba(91,142,244,0.18);color:#5B8EF4;border:1px solid rgba(91,142,244,0.3)}',
      '.bc-badge-type{background:rgba(139,92,246,0.18);color:#8B5CF6;border-color:rgba(139,92,246,0.3)}',
      '.bc-badge-dim{background:rgba(255,255,255,0.06);color:rgba(255,255,255,0.45);border-color:rgba(255,255,255,0.1)}',
      '.bc-score-row{display:flex;align-items:center;gap:6px;margin-bottom:6px}',
      '.bc-score-num{font-size:16px;font-weight:800;color:#F59E0B}',
      '.bc-score-sub{font-size:10px;color:rgba(255,255,255,0.4);margin-top:2px}',
      '.bc-score-bar-wrap{flex:1;height:4px;background:rgba(255,255,255,0.1);border-radius:2px;overflow:hidden}',
      '.bc-score-bar{height:100%;background:linear-gradient(90deg,#F59E0B,#EF4444);border-radius:2px;transition:width .6s}',
      '.bc-tags-row{display:flex;gap:4px;flex-wrap:wrap;margin-bottom:6px}',
      '.bc-tag-studio{background:rgba(34,197,94,0.12) !important;border-color:rgba(34,197,94,0.3) !important;color:#22C55E !important}',
      '.bc-synopsis{font-size:11.5px;line-height:1.6;color:rgba(255,255,255,0.7);margin-bottom:10px;background:rgba(255,255,255,0.04);border-radius:8px;padding:8px 10px;border-left:2px solid rgba(91,142,244,0.4)}',
      '.bc-footer{display:flex;align-items:center;justify-content:space-between;padding-top:8px;border-top:1px solid rgba(255,255,255,0.06)}',
      '.bc-stats{display:flex;gap:10px;font-size:11px;color:rgba(255,255,255,0.5)}',
      '.bc-link{font-size:11px;color:#5B8EF4;text-decoration:none;font-weight:600}',
      /* carte pokemon */
      '.bc-poke-layout{display:flex;gap:12px;align-items:center;margin-bottom:10px}',
      '.bc-poke-img-wrap{width:90px;height:90px;display:flex;align-items:center;justify-content:center;border-radius:12px;flex-shrink:0}',
      '.bc-poke-img{width:80px;height:80px;object-fit:contain;image-rendering:auto}',
      '.bc-poke-info{flex:1}',
      '.bc-poke-num{font-size:10px;color:rgba(255,255,255,0.35);font-weight:700;margin-bottom:2px}',
      '.bc-poke-dims{font-size:11px;color:rgba(255,255,255,0.45);margin-top:4px}',
      '.bc-poke-stats{display:grid;gap:4px;margin-top:4px}',
      '.bc-stat-row{display:flex;align-items:center;gap:6px;font-size:10.5px}',
      '.bc-stat-lbl{width:46px;color:rgba(255,255,255,0.45);flex-shrink:0;text-align:right}',
      '.bc-stat-track{flex:1;height:5px;background:rgba(255,255,255,0.08);border-radius:3px;overflow:hidden}',
      '.bc-stat-fill{height:100%;border-radius:3px;transition:width .5s}',
      '.bc-stat-val{width:28px;text-align:right;color:rgba(255,255,255,0.6);font-weight:600}',
      /* blague */
      '.bc-blague-setup{font-size:13px;color:rgba(255,255,255,0.8);margin:8px 0 4px;font-weight:500}',
      '.bc-blague-punchline{font-size:14px;font-weight:700;color:#F59E0B;margin-bottom:6px;line-height:1.4}',
      '.bc-fact-cat{font-size:10px;color:rgba(255,255,255,0.3);text-align:right;margin-top:4px}',
      /* citation */
      '.bc-citation{font-size:14px;font-style:italic;line-height:1.6;color:rgba(255,255,255,0.85);margin:10px 0 6px;padding-left:12px;border-left:3px solid #8B5CF6}',
      '.bc-citation-author{font-size:11px;color:#8B5CF6;font-weight:600;text-align:right}',
      /* pays */
      '.bc-pays-header{display:flex;align-items:center;gap:10px;margin-bottom:10px}',
      '.bc-pays-flag{width:48px;height:auto;border-radius:4px;box-shadow:0 2px 6px rgba(0,0,0,0.4)}',
      '.bc-pays-grid{display:grid;grid-template-columns:1fr 1fr;gap:6px}',
      '.bc-pays-item{display:flex;align-items:center;gap:6px;font-size:11.5px;color:rgba(255,255,255,0.7);background:rgba(255,255,255,0.04);border-radius:8px;padding:5px 8px}',
      '.bc-pays-item span{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}',
      /* bot channel post */
  ...
  function handleBotCommand(text) {
    ...
    /* ─ Anime / Manga ─ */
    var q;
    if (cmd.startsWith('/anime') || cmd.startsWith('/animé')) {
      q = args || 'demon slayer';
      p = bcFetchAnime(q);
    }
    else if (cmd.startsWith('/manga')) {
      q = args || 'berserk';
      p = bcFetchManga(q);
    }
    else if (cmd.startsWith('/webtoon') || cmd.startsWith('/manhwa') || cmd.startsWith('/manhua')) {
      var _ol = cmd.startsWith('/manhwa') ? 'ko' : cmd.startsWith('/manhua') ? 'zh' : null;
      q = args || (_ol === 'ko' ? 'solo leveling' : _ol === 'zh' ? 'martial peak' : 'tower of god');
      p = bcFetchWebtoon(q, _ol);
    }
    /* ─ GIF Hug / Pat / Cry etc ─ */
    ...
    /* ─ Pokémon ─ */
    else if (cmd.startsWith('/pokemon') || cmd.startsWith('/pokémon')) {
      q = args || 'pikachu';
      p = bcFetchPokemon(q);
    }
    /* ─ Film ─ */
    else if (cmd.startsWith('/film') || cmd.startsWith('/movie')) {
      q = args || 'interstellar';
      p = bcFetchFilm(q);
    }
    /* ─ Blague FR ─ */
    else if (cmd.startsWith('/blague') || cmd.startsWith('/joke')) {
      p = bcFetchBlague();
    }
    /* ─ Citation ─ */
    else if (cmd.startsWith('/citation') || cmd.startsWith('/quote')) {
      p = bcFetchCitation(args);
    }
    /* ─ Pays ─ */
    else if (cmd.startsWith('/pays') || cmd.startsWith('/country')) {
      q = args || 'france';
      p = bcFetchPays(q);
    }
    else if (cmd.startsWith('/paroles') || cmd.startsWith('/lyrics')) {
      if (!args) { hideBotLoading(loadId); showToast('Usage : /paroles Artiste - Titre', 'warning'); return; }
      p = bcFetchLyrics(args);
    }
    else if (cmd.startsWith('/cocktail') || cmd.startsWith('/drink')) {
      q = args || 'mojito';
      p = bcFetchCocktail(q);
    }
    else if (cmd.startsWith('/concert') || cmd.startsWith('/show')) {
      q = args || 'Paris';
      p = bcFetchConcert(q);
    }
    else if (cmd.startsWith('/artiste') || cmd.startsWith('/artist')) {
      q = args || 'Aya Nakamura';
      p = bcFetchArtiste(q);
    }
    else if (cmd.startsWith('/animequote') || cmd.startsWith('/citationanime')) {
      p = bcFetchAnimeChan(args);
    }
    else if (cmd.startsWith('/anifact')) {
      p = bcFetchAniFact();
    }
    /* ─ Aide ─ */
      ...
      sendMessage({
        type: 'text',
        text: '🤖 Commandes SIS Bot\n\n🎞 /anime · /manga · /webtoon · /manhwa · /manhua\n🎮 /pokemon · 🎬 /film · 🌍 /pays\n🍹 /cocktail · 🎵 /paroles [artiste - titre]\n🎤 /artiste · 🎪 /concert [ville]\n💬 /blague · /citation · /chuck · /fact\n🏮 /animequote · /anifact\n🤗 /hug · /pat · /kiss · /waifu\n\n→ Tape / pour voir tout'
      });
      return;
    }
    else {
  ...
  var BOT_CMDS = [
    { cmd: '/anime',       args: '[titre]',           desc: 'Fiche animé 🎞' },
    { cmd: '/manga',       args: '[titre]',           desc: 'Fiche manga 📖' },
    { cmd: '/webtoon',     args: '[titre]',           desc: 'Webtoon MangaDex 🌐' },
    { cmd: '/manhwa',      args: '[titre]',           desc: 'Manhwa coréen 🇰🇷' },
    { cmd: '/manhua',      args: '[titre]',           desc: 'Manhua chinois 🇨🇳' },
    { cmd: '/pokemon',     args: '[nom]',             desc: 'Fiche Pokémon 🎮' },
    { cmd: '/film',        args: '[titre]',           desc: 'Infos film 🎬' },
    { cmd: '/pays',        args: '[pays]',            desc: 'Infos pays 🌍' },
    { cmd: '/paroles',     args: '[artiste - titre]', desc: 'Paroles chanson 🎵' },
    { cmd: '/cocktail',    args: '[nom]',             desc: 'Recette cocktail 🍹' },
    { cmd: '/artiste',     args: '[nom]',             desc: 'Artiste musical 🎤' },
    { cmd: '/concert',     args: '[ville]',           desc: 'Concerts à venir 🎪' },
    { cmd: '/animequote',  args: '[anime?]',          desc: 'Citation animé 🏮' },
    { cmd: '/anifact',     args: '',                  desc: 'Fait insolite animé 🐉' },
    { cmd: '/blague',      args: '',                  desc: 'Blague FR 😂' },
    { cmd: '/citation',    args: '[thème?]',          desc: 'Citation inspirante 💬' },
    { cmd: '/chuck',       args: '',                  desc: 'Blague Chuck Norris 💪' },
    { cmd: '/fact',        args: '',                  desc: 'Fait insolite 💡' },
    { cmd: '/meme',        args: '',                  desc: 'Mème aléatoire 🎭' },
    { cmd: '/recette',     args: '[plat?]',           desc: 'Recette cuisine 🍽️' },
    { cmd: '/hug',         args: '',                  desc: 'GIF hug 🤗' },
    { cmd: '/pat',         args: '',                  desc: 'GIF pat 🥺' },
    { cmd: '/kiss',        args: '',                  desc: 'GIF kiss 💋' },
    { cmd: '/cry',         args: '',                  desc: 'GIF cry 😢' },
    { cmd: '/wave',        args: '',                  desc: 'GIF wave 👋' },
    { cmd: '/waifu',       args: '',                  desc: 'Waifu aléatoire 🌸' },
    { cmd: '/aide',        args: '',                  desc: 'Toutes les commandes' }
  ];
  ...
  function bcFetchAnime(q) {
    return fetch('https://api.jikan.moe/v4/anime?q=' + encodeURIComponent(q) + '&limit=1')
      ...
      .then(function(d) {
        ...
        var a = d.data[0];
        var synopsis = (a.synopsis||'').replace(/\[Written.*?\]/g,'').replace(/\(Source.*?\)/gi,'').trim().substring(0,220);
        return {
          cardType: 'anime',
          ...
          status: a.status,
          synopsis: synopsis,
          genres: (a.genres||[]).slice(0,4).map(function(g){
            var GENRE_FR = {'Action':'Action','Adventure':'Aventure','Avant Garde':'Avant-Garde','Award Winning':'Primé','Comedy':'Comédie','Drama':'Drame','Fantasy':'Fantaisie','Horror':'Horreur','Gourmet':'Gastronomie','Mystery':'Mystère','Romance':'Romance','Sci-Fi':'Sci-Fi','Sports':'Sport','Supernatural':'Surnaturel','Suspense':'Suspense','Ecchi':'Ecchi','Erotica':'Érotique','Hentai':'Hentai','Slice of Life':'Tranche de vie','School':'Scolaire','Historical':'Historique','Military':'Militaire','Music':'Musique','Parody':'Parodie','Psychological':'Psychologique','Super Power':'Super pouvoir','Vampire':'Vampire','Demons':'Démons','Magic':'Magie','Mecha':'Mecha','Space':'Espace','Samurai':'Samouraï','Martial Arts':'Arts martiaux','Police':'Police','Josei':'Josei','Shounen':'Shōnen','Shoujo':'Shōjo','Seinen':'Seinen','Thriller':'Thriller','Racing':'Course'};
            return GENRE_FR[g.name] || g.name;
          }),
          studios: (a.studios||[]).slice(0,2).map(function(s){ return s.name; }),
          year: a.year,
          season: a.season,
          url: a.url,
          type: a.type,
          rating: (function(r) {
            var RATING_FR = {'G - All Ages':'Tout public','PG - Children':'Enfants','PG-13 - Teens 13 or older':'Dès 13 ans','R - 17+ (violence & profanity)':'+17 ans','R+ - Mild Nudity':'+17 ans (adult)','Rx - Hentai':'Hentai'};
            return RATING_FR[r] || r || '';
          })(a.rating)
        };
      });
  }

  function bcFetchWebtoon(q, origLang) {
    /* Brackets %5B%5D encodés — requis par MangaDex */
    var p = [
    ...
    if (origLang) p.push('originalLanguage%5B%5D=' + origLang);
    return fetch('https://api.mangadex.org/manga?' + p.join('&'))
      .then(function(r) { if (!r.ok) throw new Error('HTTP ' + r.status); return r.json(); })
      .then(function(d) {
        if (!d.data || !d.data.length) throw new Error('Introuvable : ' + q);
        /* Priorité résultat FR */
        var chosen = d.data[0];
        for (var i = 0; i < d.data.length; i++) {
          if ((d.data[i].attributes.availableTranslatedLanguages||[]).indexOf('fr') !== -1) { chosen = d.data[i]; break; }
        }
        var m = chosen, attr = m.attributes, rel = m.relationships || [];
        /* Titre */
        var t = attr.title || {};
        var title = t.fr || t.en || t['ja-ro'] || t['ko-ro'] || Object.values(t)[0] || q;
        /* Synopsis — nettoyer balises MangaDex [b][i][spoiler] */
        var synopsis = '';
        if (attr.description) {
          var raw = attr.description.fr || attr.description.en || Object.values(attr.description)[0] || '';
          raw = raw.replace(/\[(\/?)(b|i|s|u|spoiler|url)[^\]]*\]/gi, '').trim();
          synopsis = raw.length > 240 ? raw.substring(0, 240) + '…' : raw;
        }
        /* Cover .512.jpg */
        var cover = '';
        for (var ci = 0; ci < rel.length; ci++) {
          if (rel[ci].type === 'cover_art' && rel[ci].attributes && rel[ci].attributes.fileName) {
            cover = 'https://uploads.mangadex.org/covers/' + m.id + '/' + rel[ci].attributes.fileName + '.512.jpg';
            break;
          }
        }
        /* Auteurs */
        var authors = [];
        for (var ai = 0; ai < rel.length; ai++) {
          if ((rel[ai].type === 'author' || rel[ai].type === 'artist') && rel[ai].attributes && rel[ai].attributes.name) {
            if (authors.indexOf(rel[ai].attributes.name) === -1) authors.push(rel[ai].attributes.name);
            if (authors.length >= 2) break;
          }
        }
        /* Tags FR */
        var TAG_FR = {'Action':'Action','Adventure':'Aventure','Comedy':'Comédie','Drama':'Drame','Fantasy':'Fantaisie','Horror':'Horreur','Mystery':'Mystère','Romance':'Romance','Sci-Fi':'Sci-Fi','Slice of Life':'Tranche de vie','Historical':'Historique','Psychological':'Psychologique','Martial Arts':'Arts martiaux','Thriller':'Thriller','Isekai':'Isekai','Magic':'Magie','Supernatural':'Surnaturel','Sports':'Sport','Reincarnation':'Réincarnation','Survival':'Survie','Tragedy':'Tragédie'};
        var tags = (attr.tags||[]).slice(0,4).map(function(tg) {
          if (!tg.attributes || !tg.attributes.name) return '';
          var n = tg.attributes.name.en || Object.values(tg.attributes.name)[0] || '';
          return TAG_FR[n] || n;
        }).filter(Boolean);
        /* Origine */
        var ORIGIN = {ja:'🇯🇵 Manga', ko:'🇰🇷 Manhwa', zh:'🇨🇳 Manhua', 'zh-hk':'🇭🇰 Manhua', en:'🇺🇸 Comic'};
        var originType = ORIGIN[attr.originalLanguage||''] || '📖 Webtoon';
        var STATUS_FR = {completed:'✅ Terminé', ongoing:'🔴 En cours', hiatus:'⏸ Pause', cancelled:'🚫 Annulé'};
        var avail = attr.availableTranslatedLanguages || [];
        var langBadges = [];
        if (avail.indexOf('fr') !== -1) langBadges.push('🇫🇷');
        if (avail.indexOf('en') !== -1) langBadges.push('🇬🇧');
        var extra = avail.filter(function(l){ return l!=='fr'&&l!=='en'; }).length;
        if (extra) langBadges.push('+'+extra);
        return {
          cardType:'webtoon', label:title, title:title, image:cover, synopsis:synopsis,
          tags:tags, authors:authors, status:STATUS_FR[attr.status]||attr.status||'',
          year:attr.year||null, lastChapter:attr.lastChapter||null, lastVolume:attr.lastVolume||null,
          originType:originType, langBadges:langBadges,
  ...
  function bcFetchChuck() {
    /* Chuck Norris — pas d'API FR officielle, on utilise des faits traduits */
    var CHUCK_FR = [
      "Chuck Norris n'a pas besoin de Google. Il sait déjà tout.",
      "Quand Chuck Norris entre dans une pièce, les murs reculent.",
      "Chuck Norris compte jusqu'à l'infini. Deux fois.",
      "Chuck Norris a déjà battu le soleil au bras de fer.",
      "La mort a peur de croiser Chuck Norris dans la rue.",
      "Chuck Norris peut éteindre un incendie avec de l'essence.",
      "Chuck Norris n'a jamais de bugs dans son code. Les bugs ont peur de lui.",
      "Quand Chuck Norris fait du push-up, c'est la Terre qui descend.",
      "Chuck Norris peut diviser un nombre par zéro.",
      "Chuck Norris a déjà joué à la roulette russe avec un pistolet à eau — et gagné.",
      "Chuck Norris n'a pas besoin de montre. Il décide lui-même quelle heure il est.",
      "Dieu a dit « Que la lumière soit ». Chuck Norris a répondu « S'il te plaît ».",
      "Chuck Norris peut entendre les sourds.",
      "Le courant continu a peur de Chuck Norris.",
      "Chuck Norris a déjà gagné au Uno avec un +4 blanc.",
      "Chuck Norris peut souffler dans une bougie et allumer un incendie.",
      "Quand Chuck Norris marche dans la rue, c'est le trottoir qui fait attention.",
      "Chuck Norris a déjà envoyé un SMS à quelqu'un avant qu'il n'ait son téléphone.",
      "Chuck Norris peut mettre le feu à de l'eau.",
      "La gravité ne s'applique pas à Chuck Norris. Il s'applique à elle."
    ];
    var text = CHUCK_FR[Math.floor(Math.random() * CHUCK_FR.length)];
  ...
  function bcFetchFact() {
    return fetch('https://uselessfacts.jsph.pl/api/v2/facts/random?language=fr')
      .then(function(r){ return r.json(); })
      .then(function(d) {
        return {
          cardType: 'fact',
  ...
  /* ── Pokémon ── */
  function bcFetchPokemon(q) {
    var slug = q.toLowerCase().trim().replace(/\s+/g,'-');
    return fetch('https://pokeapi.co/api/v2/pokemon/' + encodeURIComponent(slug))
      .then(function(r){ if(!r.ok) throw new Error('Pokémon introuvable'); return r.json(); })
      .then(function(p) {
        return fetch('https://pokeapi.co/api/v2/pokemon-species/' + p.id)
          .then(function(r2){ return r2.json(); })
          .then(function(sp) {
            var desc = '';
            for(var i=0;i<(sp.flavor_text_entries||[]).length;i++){
              if(sp.flavor_text_entries[i].language.name==='fr'){
                desc = sp.flavor_text_entries[i].flavor_text.replace(/\f|\n/g,' ');
                break;
              }
            }
            var nameFr = '';
            for(var j=0;j<(sp.names||[]).length;j++){
              if(sp.names[j].language.name==='fr'){ nameFr = sp.names[j].name; break; }
            }
            var types = p.types.map(function(t){return t.type.name;});
            var TYPE_FR = {normal:'Normal',fire:'Feu',water:'Eau',grass:'Plante',electric:'Electrik',ice:'Glace',fighting:'Combat',poison:'Poison',ground:'Sol',flying:'Vol',psychic:'Psy',bug:'Insecte',rock:'Roche',ghost:'Spectre',dragon:'Dragon',dark:'Ténèbres',steel:'Acier',fairy:'Fée'};
            var TYPE_COLOR = {fire:'#EF4444',water:'#3B82F6',grass:'#22C55E',electric:'#F59E0B',psychic:'#EC4899',ice:'#67E8F9',dragon:'#7C3AED',dark:'#9CA3AF',fairy:'#F9A8D4',normal:'#9CA3AF',ghost:'#7C3AED',steel:'#6B7280',fighting:'#B45309',poison:'#A855F7',ground:'#D97706',flying:'#60A5FA',bug:'#84CC16',rock:'#A16207'};
            var mainType = types[0];
            var stats = {};
            p.stats.forEach(function(s){ stats[s.stat.name] = s.base_stat; });
            return {
              cardType: 'pokemon',
              label: '🎮 ' + (nameFr||p.name),
              id: p.id,
              name: nameFr || p.name.charAt(0).toUpperCase()+p.name.slice(1),
              nameEn: p.name,
              image: (p.sprites.other && p.sprites.other['official-artwork'] && p.sprites.other['official-artwork'].front_default) || p.sprites.front_default,
              types: types.map(function(t){ return { name: TYPE_FR[t]||t, color: TYPE_COLOR[t]||'#5B8EF4' }; }),
              mainColor: TYPE_COLOR[mainType] || '#5B8EF4',
              desc: desc,
              hp: stats['hp']||0, atk: stats['attack']||0, def: stats['defense']||0,
              spatk: stats['special-attack']||0, spdef: stats['special-defense']||0, speed: stats['speed']||0,
  ...
  /* ── Film (Open Movie Database — sans clé via proxy OMDb) ── */
  function bcFetchFilm(q) {
    return fetch('https://api.themoviedb.org/3/search/movie?query=' + encodeURIComponent(q) + '&language=fr-FR&api_key=8265bd1679663a7ea12ac168da84d2e8')
      .then(function(r){ return r.json(); })
      .then(function(d) {
        var m = d.results && d.results[0];
        if (!m) throw new Error('Film introuvable : ' + q);
        return fetch('https://api.themoviedb.org/3/movie/'+m.id+'?language=fr-FR&api_key=8265bd1679663a7ea12ac168da84d2e8&append_to_response=credits')
          .then(function(r2){ return r2.json(); })
          .then(function(detail) {
            var genres = (detail.genres||[]).slice(0,3).map(function(g){return g.name;});
            var directors = [];
            if (detail.credits && detail.credits.crew) {
              detail.credits.crew.forEach(function(c){ if(c.job==='Director') directors.push(c.name); });
            }
            return {
              cardType: 'film',
              label: '🎬 ' + (detail.title||q),
              title: detail.title,
              titleOriginal: detail.original_title,
              image: detail.poster_path ? 'https://image.tmdb.org/t/p/w342'+detail.poster_path : '',
              backdrop: detail.backdrop_path ? 'https://image.tmdb.org/t/p/w780'+detail.backdrop_path : '',
              synopsis: (detail.overview||'').substring(0,200),
  ...
  /* ── Blague FR (JokeAPI) ── */
  function bcFetchBlague() {
    return fetch('https://v2.jokeapi.dev/joke/Any?lang=fr&blacklistFlags=nsfw,racist,sexist&type=twopart,single')
      .then(function(r){ return r.json(); })
      .then(function(d) {
        if (d.error) throw new Error('Blague introuvable');
        return {
          cardType: 'blague',
          label: '😂 Blague',
          setup: d.type === 'twopart' ? d.setup : '',
          punchline: d.type === 'twopart' ? d.delivery : d.joke,
          category: d.category
        };
      });
  }
  ...
  function bcFetchCitation(theme) {
    var QUOTES_FR = [
      ...
      {text:"La connaissance s'acquiert par l'expérience, tout le reste n'est qu'information.", author:'Albert Einstein'},
      {text:"Ne juge pas chaque jour par la récolte que tu fais, mais par les graines que tu plantes.", author:'Robert Louis Stevenson'},
      {text:"Ce n'est pas la montagne que nous conquérons, mais nous-mêmes.", author:'Edmund Hillary'},
      {text:"Je n'ai pas échoué. J'ai juste trouvé 10 000 façons qui ne fonctionnent pas.", author:'Thomas Edison'},
      {text:"Si vous pouvez rêver de quelque chose, vous pouvez le réaliser.", author:'Walt Disney'},
      {text:"Le courage n'est pas l'absence de peur, c'est la capacité de la surmonter.", author:'Nelson Mandela'},
      {text:"Chaque expert a un jour été un débutant.", author:'Helen Hayes'},
      {text:"La seule limite à nos réalisations de demain sont nos doutes d'aujourd'hui.", author:'Franklin D. Roosevelt'},
      {text:"Ce qui ne me tue pas me rend plus fort.", author:'Friedrich Nietzsche'},
      {text:"Mieux vaut allumer une bougie que maudire l'obscurité.", author:'Eleanor Roosevelt'},
      {text:"Le génie c'est 1% d'inspiration et 99% de transpiration.", author:'Thomas Edison'},
      {text:"Agis comme si ce que tu fais faisait une différence. Ça en fait une.", author:'William James'},
      {text:"Le monde appartient à ceux qui se lèvent tôt.", author:'Proverbe français'}
    ];
    /* zenquotes.io — gratuit, sans clé, CORS ok */
    return fetch('https://zenquotes.io/api/random')
      .then(function(r) { if (!r.ok) throw new Error('HTTP ' + r.status); return r.json(); })
      .then(function(d) {
        var item = Array.isArray(d) ? d[0] : d;
        if (!item || !item.q) throw new Error('Format inattendu');
        return { cardType:'citation', label:'💬 Citation', text:item.q, author:item.a || 'Inconnu', tags:theme?[theme]:[], lang:'en' };
      })
      .catch(function() {
        /* Fallback FR hardcodé */
        var pool = QUOTES_FR;
        if (theme) {
          var f = QUOTES_FR.filter(function(x){ return x.text.toLowerCase().indexOf(theme.toLowerCase()) !== -1 || x.author.toLowerCase().indexOf(theme.toLowerCase()) !== -1; });
          if (f.length) pool = f;
        }
        var q = pool[Math.floor(Math.random() * pool.length)];
        return { cardType:'citation', label:'💬 Citation', text:q.text, author:q.author, tags:theme?[theme]:[], lang:'fr' };
      });
  }

  /* ── Pays (REST Countries) ── */
  function bcFetchPays(q) {
    return fetch('https://restcountries.com/v3.1/name/' + encodeURIComponent(q) + '?fields=name,capital,population,area,flags,currencies,languages,region,subregion,timezones,cca2')
      .then(function(r){ if(!r.ok) throw new Error('Pays introuvable'); return r.json(); })
      .then(function(arr) {
        var c = arr[0];
        var langList = Object.values(c.languages||{}).join(', ');
        var currency = Object.values(c.currencies||{}).map(function(v){return v.name+' ('+v.symbol+')';}).join(', ');
        var pop = c.population >= 1e6 ? (c.population/1e6).toFixed(1)+' M' : (c.population/1e3).toFixed(0)+' k';
        var area = c.area >= 1e6 ? (c.area/1e6).toFixed(2)+' M km²' : c.area.toLocaleString()+' km²';
        return {
          cardType: 'pays',
          label: '🌍 ' + (c.name.translations && c.name.translations.fra ? c.name.translations.fra.common : c.name.common),
          name: c.name.translations && c.name.translations.fra ? c.name.translations.fra.common : c.name.common,
  ...
  /* ── Manga — Jikan /manga endpoint ── */
  function bcFetchManga(q) {
    return fetch('https://api.jikan.moe/v4/manga?q=' + encodeURIComponent(q) + '&limit=1')
      .then(function(r){ if(!r.ok) throw new Error('Jikan ' + r.status); return r.json(); })
      .then(function(d) {
        if (!d.data || !d.data[0]) throw new Error('Manga introuvable : ' + q);
        var a = d.data[0];
        var GENRE_FR = {'Action':'Action','Adventure':'Aventure','Comedy':'Comédie','Drama':'Drame','Fantasy':'Fantaisie','Horror':'Horreur','Mystery':'Mystère','Romance':'Romance','Sci-Fi':'Sci-Fi','Sports':'Sport','Supernatural':'Surnaturel','Slice of Life':'Tranche de vie','School':'Vie scolaire','Historical':'Historique','Psychological':'Psychologique','Martial Arts':'Arts martiaux','Thriller':'Thriller','Isekai':'Isekai','Magic':'Magie','Shounen':'Shōnen','Shoujo':'Shōjo','Seinen':'Seinen','Josei':'Josei'};
        var STATUS_FR = {'Finished':'✅ Terminé','Publishing':'🔴 En cours','On Hiatus':'⏸ Pause','Discontinued':'🚫 Arrêté','Not yet published':'🟡 À venir'};
        var synopsis = (a.synopsis||'').replace(/\[Written.*?\]/g,'').replace(/\(Source.*?\)/gi,'').trim();
        if (synopsis.length > 220) synopsis = synopsis.substring(0, 220) + '…';
        return {
          cardType:'manga', label:'📖 ' + (a.title_french||a.title_english||a.title),
          title: a.title_french||a.title_english||a.title,
          titleOriginal: a.title, titleJp: a.title_japanese||'',
          image: a.images&&a.images.jpg ? (a.images.jpg.large_image_url||a.images.jpg.image_url) : '',
          score:a.score, rank:a.rank, volumes:a.volumes, chapters:a.chapters,
          status: STATUS_FR[a.status]||a.status||'',
          synopsis: synopsis,
          genres: (a.genres||[]).concat(a.themes||[]).slice(0,4).map(function(g){ return GENRE_FR[g.name]||g.name; }),
          authors: (a.authors||[]).slice(0,2).map(function(au){ return au.name.split(', ').reverse().join(' '); }),
          demographics: (a.demographics||[]).slice(0,1).map(function(d){ return GENRE_FR[d.name]||d.name; }),
          type: a.type||'Manga', url: a.url,
          year: a.published&&a.published.prop&&a.published.prop.from ? a.published.prop.from.year : null
        };
      });
  }

  /* ── Paroles — lyrics.ovh + fallback ── */
  function bcFetchLyrics(q) {
    var artist = '', title = '';
    var dash = q.indexOf(' - ');
    if (dash !== -1) { artist = q.substring(0, dash).trim(); title = q.substring(dash + 3).trim(); }
    else { var sp = q.indexOf(' '); artist = sp > 0 ? q.substring(0, sp) : q; title = sp > 0 ? q.substring(sp + 1) : ''; }
    if (!title) return Promise.reject(new Error('Format : /paroles Artiste - Titre'));
    var primary  = 'https://api.lyrics.ovh/v1/' + encodeURIComponent(artist) + '/' + encodeURIComponent(title);
    var fallback = 'https://some-random-api.com/lyrics?title=' + encodeURIComponent(title) + '&artist=' + encodeURIComponent(artist);
    return fetch(primary)
      .then(function(r) { if(!r.ok) throw new Error('primary fail'); return r.json(); })
      .then(function(d) { if(!d.lyrics) throw new Error('no lyrics'); return d.lyrics; })
      .catch(function() {
        return fetch(fallback)
          .then(function(r2) { return r2.json(); })
          .then(function(d2) { if(!d2.lyrics) throw new Error('Paroles introuvables : ' + q); return d2.lyrics; });
      })
      .then(function(lyrics) {
        var clean = (lyrics||'').replace(/\r/g, '').trim();
        return { cardType:'lyrics', label:'🎵 ' + artist + ' — ' + title, artist:artist, title:title, lyrics:clean, lines:clean.split('\n').length };
      });
  }

  /* ── Cocktail — TheCocktailDB (sans clé, v1 gratuite) ── */
  function bcFetchCocktail(q) {
    return fetch('https://www.thecocktaildb.com/api/json/v1/1/search.php?s=' + encodeURIComponent(q))
      .then(function(r){ return r.json(); })
      .then(function(d) {
        if (!d.drinks || !d.drinks[0]) return fetch('https://www.thecocktaildb.com/api/json/v1/1/random.php').then(function(r2){ return r2.json(); }).then(function(d2){ return { drinks: d2.drinks }; });
        return d;
      })
      .then(function(d) {
        var c = d.drinks[0];
        ...
        for (var i = 1; i <= 15; i++) {
          ...
          var mes = c['strMeasure' + i];
          if (ing && ing.trim()) ingredients.push((mes ? mes.trim() + ' ' : '') + ing.trim());
        }
        var instructions = (c.strInstructionsFR || c.strInstructions || '').substring(0, 200);
        return {
          cardType:'cocktail', label:'🍹 ' + c.strDrink,
          name: c.strDrink, image: c.strDrinkThumb,
          category: c.strCategory, alcoholic: c.strAlcoholic,
          glass: c.strGlass, ingredients: ingredients.slice(0, 8),
          instructions: instructions + (instructions.length === 200 ? '…' : ''),
          url: 'https://www.thecocktaildb.com/drink/' + c.idDrink
        };
      });
  }

  /* ── Concert — Bandsintown (sans clé) ── */
  function bcFetchConcert(city) {
    var appId = 'sis-app';
    var url = 'https://rest.bandsintown.com/v3.1/events/search?location=' + encodeURIComponent(city) + '&date=upcoming&per_page=5&app_id=' + appId;
    return fetch(url)
      .then(function(r) { if (!r.ok) throw new Error('HTTP ' + r.status); return r.json(); })
      .then(function(d) {
        var events = Array.isArray(d) ? d : [];
        if (!events.length) throw new Error('Aucun concert trouvé à ' + city);
        return {
          cardType:'concert', label:'🎪 Concerts à ' + city,
          city: city, events: events.slice(0,5).map(function(e) {
            return {
              artist: e.artist && e.artist.name ? e.artist.name : 'Artiste inconnu',
              venue: e.venue ? (e.venue.name + ', ' + (e.venue.city||city)) : city,
              date: e.datetime ? e.datetime.substring(0,10) : '',
              url: e.url || ''
            };
          })
        };
  ...
  /* ── Artiste — MusicBrainz (sans clé) ── */
  function bcFetchArtiste(q) {
    return fetch('https://musicbrainz.org/ws/2/artist/?query=' + encodeURIComponent(q) + '&limit=1&fmt=json', { headers:{ 'User-Agent':'SIS-App/1.0 (contact@sis.app)' } })
      .then(function(r){ return r.json(); })
      .then(function(d) {
        var a = d.artists && d.artists[0];
        if (!a) throw new Error('Artiste introuvable : ' + q);
        var TYPE_FR = {'Person':'Artiste solo','Group':'Groupe','Orchestra':'Orchestre','Choir':'Chœur','Character':'Personnage fictif','Other':'Autre'};
        var genres = (a.tags||[]).sort(function(x,y){ return (y.count||0)-(x.count||0); }).slice(0,4).map(function(t){ return t.name; });
        return {
          cardType:'artiste', label:'🎤 ' + a.name,
          name: a.name, type: TYPE_FR[a.type]||a.type||'',
          country: a.country||'', beginYear: a.life-span&&a['life-span']['begin'] ? a['life-span']['begin'].substring(0,4) : '',
          genres: genres, disambiguation: a.disambiguation||'',
          url: 'https://musicbrainz.org/artist/' + a.id
        };
      });
  }

  /* ── AnimeChan — citation de personnage animé ── */
  function bcFetchAnimeChan(anime) {
    var url = anime ? 'https://animechan.xyz/api/random/anime?title=' + encodeURIComponent(anime) : 'https://animechan.xyz/api/random';
    return fetch(url)
      .then(function(r){ if(!r.ok) throw new Error('HTTP ' + r.status); return r.json(); })
      .then(function(d) {
        /* AnimeChan peut retourner tableau ou objet */
        var item = Array.isArray(d) ? d[0] : d;
        if (!item || !item.quote) throw new Error('Citation introuvable');
        return {
          cardType:'animequote', label:'🏮 ' + (item.character||'?'),
          quote: item.quote, character: item.character||'Inconnu', anime: item.anime||''
        };
      });
  }

  /* ── AnimeFacts ── */
  function bcFetchAniFact() {
    return fetch('https://animefacts.net/api/v1/facts/random')
      .then(function(r){ if(!r.ok) throw new Error('HTTP ' + r.status); return r.json(); })
      .then(function(d) {
        var fact = d.fact || d.data || d.text || (typeof d === 'string' ? d : '');
        if (!fact) throw new Error('Fait introuvable');
        return { cardType:'anifact', label:'🐉 Fait anime', fact: fact };
      })
      .catch(function() {
        /* Fallback hardcodé si API down */
        var FACTS = [
          "Le mot 'anime' est simplement l'abréviation japonaise du mot 'animation'.",
          "Dragon Ball Z a popularisé les transformations de puissance (Super Saiyan) dans les années 90.",
          "One Piece est le manga le plus vendu de tous les temps avec plus de 500 millions d'exemplaires.",
          "L'animé Spirited Away est le film d'animation ayant rapporté le plus d'argent au Japon.",
          "Attack on Titan a été refusé par 7 éditeurs avant d'être publié.",
          "Le studio Ghibli possède une règle : jamais de happy ending forcé.",
          "Naruto a couru pendant plus de 500 épisodes sans jamais vraiment changer de technique.",
          "Fullmetal Alchemist Brotherhood est régulièrement noté comme le meilleur animé sur MyAnimeList.",
          "Le réalisateur de Demon Slayer a pleuré en regardant les épisodes finaux de sa propre série.",
          "Death Note a été interdit dans plusieurs écoles au Japon à cause de cahiers remplis de noms."
        ];
        var f = FACTS[Math.floor(Math.random() * FACTS.length)];
        return { cardType:'anifact', label:'🐉 Fait anime', fact: f };
      });
  }
  ...
  function buildBotCardHtml(data) {
    ...
    if (ct === 'anime') {
      var statusTxt = cd.status === 'Finished Airing' ? '✅ Terminé' : cd.status === 'Currently Airing' ? '🔴 En cours' : cd.status === 'Not yet aired' ? '🟡 À venir' : (cd.status||'');
      var typeMap = {TV:'Série TV', Movie:'Film', OVA:'OVA', ONA:'ONA', Special:'Spécial', Music:'Clip'};
      var typeTxt = typeMap[cd.type] || cd.type || '';
      var seasonMap = {spring:'Printemps',summer:'Été',fall:'Automne',winter:'Hiver'};
      var airInfo = [(cd.season ? seasonMap[cd.season]||cd.season : ''), cd.year||''].filter(Boolean).join(' ');
      var genres = (cd.genres||[]).map(function(g){ return '<span class="bc-tag">'+escapeHtml(g)+'</span>'; }).join('');
      var studios = (cd.studios||[]).map(function(s){ return '<span class="bc-tag bc-tag-studio">'+escapeHtml(s)+'</span>'; }).join('');
      var imgHtml = cd.image ? '<img class="bc-anime-cover" src="'+escapeHtml(cd.image)+'" loading="lazy" alt="cover">' : '';
      var scoreBar = '';
      if (cd.score) {
        var pct = Math.round(cd.score * 10);
        scoreBar = '<div class="bc-score-bar-wrap"><div class="bc-score-bar" style="width:'+pct+'%"></div></div>';
      }
      return '<div class="bot-card bc-card-anime">'+
        '<div class="bc-stripe" style="background:linear-gradient(90deg,#8B5CF6,#5B8EF4)"></div>'+
        '<div class="bc-body">'+
          cmdLine+
          '<div class="bc-anime-layout">'+
            (imgHtml ? '<div class="bc-anime-poster">'+imgHtml+'</div>' : '')+
            '<div class="bc-anime-info">'+
              '<div class="bc-anime-title">'+escapeHtml(cd.title||'')+'</div>'+
              (cd.titleEn && cd.title !== cd.titleEn ? '<div class="bc-anime-subtitle">'+escapeHtml(cd.titleEn)+'</div>' : '')+
              (cd.titleJp ? '<div class="bc-anime-jp">'+escapeHtml(cd.titleJp)+'</div>' : '')+
              '<div class="bc-anime-meta-row">'+
                (typeTxt ? '<span class="bc-badge bc-badge-type">'+typeTxt+'</span>' : '')+
                (statusTxt ? '<span class="bc-badge">'+statusTxt+'</span>' : '')+
                (airInfo ? '<span class="bc-badge bc-badge-dim">📅 '+escapeHtml(airInfo)+'</span>' : '')+
              '</div>'+
              (cd.score ? '<div class="bc-score-row"><span class="bc-score-num">⭐ '+cd.score+'</span><span class="bc-score-sub">/10</span>'+scoreBar+'</div>' : '')+
              '<div class="bc-tags-row">'+genres+studios+'</div>'+
            '</div>'+
          '</div>'+
          (cd.synopsis ? '<div class="bc-synopsis">'+escapeHtml(cd.synopsis)+'</div>' : '')+
          '<div class="bc-footer">'+
            '<div class="bc-stats">'+
              (cd.episodes ? '<span>📺 '+cd.episodes+' ép.</span>' : '')+
              (cd.duration ? '<span>⏱ '+escapeHtml(cd.duration.replace(' per ep.','').replace(' per ep',''))+'</span>' : '')+
              (cd.rank ? '<span>🏆 #'+cd.rank+'</span>' : '')+
            '</div>'+
            (cd.url ? '<a class="bc-link" href="'+escapeHtml(cd.url)+'" target="_blank" rel="noopener">MyAnimeList →</a>' : '')+
          '</div>'+
        '</div>'+
      '</div>';
    }
    ...
    if (ct === 'manga') {
      var imgHtml = cd.image ? '<img class="bc-anime-cover" src="'+escapeHtml(cd.image)+'" loading="lazy" alt="cover">' : '';
      var genres  = (cd.genres||[]).map(function(g){ return '<span class="bc-tag">'+escapeHtml(g)+'</span>'; }).join('');
      var typeMap = {Manga:'Manga',Manhwa:'Manhwa',Manhua:'Manhua',Novel:'Light Novel','One-shot':'One-shot'};
      var typeTxt = typeMap[cd.type]||cd.type||'Manga';
      var demoBadge = cd.demographics&&cd.demographics[0] ? '<span class="bc-badge bc-badge-dim">'+escapeHtml(cd.demographics[0])+'</span>' : '';
      return '<div class="bot-card"><div class="bc-stripe" style="background:linear-gradient(90deg,#10B981,#3B82F6)"></div><div class="bc-body">'+
        cmdLine+
        '<div class="bc-anime-inner">'+imgHtml+
          '<div class="bc-anime-info">'+
            '<div class="bc-anime-title">'+escapeHtml(cd.title||'')+'</div>'+
            (cd.titleOriginal&&cd.title!==cd.titleOriginal?'<div class="bc-anime-sub">'+escapeHtml(cd.titleOriginal)+'</div>':'')+
            (cd.titleJp?'<div class="bc-anime-sub" style="font-style:italic;font-size:10px">'+escapeHtml(cd.titleJp)+'</div>':'')+
            '<div class="bc-tags" style="margin:4px 0">'+
              '<span class="bc-badge bc-badge-type">'+typeTxt+'</span>'+
              (cd.status?'<span class="bc-badge">'+cd.status+'</span>':'')+
              demoBadge+
            '</div>'+
            (cd.score?'<div class="bc-anime-score">⭐ '+cd.score+'/10</div>':'')+
            '<div class="bc-tags">'+genres+'</div>'+
            (cd.authors&&cd.authors.length?'<div class="bc-anime-sub">✍️ '+escapeHtml(cd.authors.join(', '))+'</div>':'')+
          '</div>'+
        '</div>'+
        (cd.synopsis?'<div class="bc-anime-synopsis" style="-webkit-line-clamp:4;margin-top:8px;padding:6px 8px;background:rgba(255,255,255,0.04);border-radius:6px;display:-webkit-box;overflow:hidden">'+escapeHtml(cd.synopsis)+'</div>':'')+
        '<div class="bc-anime-footer">'+
          (cd.volumes?'<span>📚 '+cd.volumes+' vol.</span>':'')+
          (cd.chapters?'<span>📄 '+cd.chapters+' ch.</span>':'')+
          (cd.rank?'<span>🏆 #'+cd.rank+'</span>':'')+
    ...
    if (ct === 'webtoon') {
      ...
      var langHtml = (cd.langBadges||[]).map(function(l){ return '<span class="bc-badge">'+escapeHtml(l)+'</span>'; }).join('');
      return '<div class="bot-card"><div class="bc-stripe" style="background:linear-gradient(90deg,#EC4899,#8B5CF6)"></div><div class="bc-body">'+
        cmdLine+
        '<div class="bc-anime-inner">'+imgHtml+
          '<div class="bc-anime-info">'+
            '<div class="bc-anime-title">'+escapeHtml(cd.title||'')+'</div>'+
            '<div class="bc-tags" style="margin:4px 0">'+
              (cd.originType?'<span class="bc-badge bc-badge-type">'+escapeHtml(cd.originType)+'</span>':'')+
              (cd.status?'<span class="bc-badge">'+escapeHtml(cd.status)+'</span>':'')+
              (cd.year?'<span class="bc-badge bc-badge-dim">📅 '+cd.year+'</span>':'')+
            '</div>'+
            (cd.authors&&cd.authors.length?'<div class="bc-anime-sub">✍️ '+escapeHtml(cd.authors.join(', '))+'</div>':'')+
            '<div class="bc-tags">'+tags+langHtml+'</div>'+
          '</div>'+
        '</div>'+
        (cd.synopsis?'<div class="bc-anime-synopsis" style="-webkit-line-clamp:4;margin-top:8px;padding:6px 8px;background:rgba(255,255,255,0.04);border-radius:6px;display:-webkit-box;overflow:hidden">'+escapeHtml(cd.synopsis)+'</div>':'')+
        '<div class="bc-anime-footer">'+
          (cd.lastChapter?'<span>📄 Ch.'+escapeHtml(String(cd.lastChapter))+'</span>':'')+
          (cd.lastVolume?'<span>📚 Vol.'+escapeHtml(String(cd.lastVolume))+'</span>':'')+
    ...
    /* Fallback */
    if (ct === 'pokemon') {
      var typeChips = (cd.types||[]).map(function(t){ return '<span class="bc-tag" style="background:'+t.color+'22;border-color:'+t.color+'55;color:'+t.color+'">'+escapeHtml(t.name)+'</span>'; }).join('');
      var statBar = function(label, val, color) {
        return '<div class="bc-stat-row"><span class="bc-stat-lbl">'+label+'</span><div class="bc-stat-track"><div class="bc-stat-fill" style="width:'+Math.min(100,Math.round(val/255*100))+'%;background:'+color+'"></div></div><span class="bc-stat-val">'+val+'</span></div>';
      };
      return '<div class="bot-card"><div class="bc-stripe" style="background:linear-gradient(90deg,'+(cd.mainColor||'#5B8EF4')+',#8B5CF6)"></div><div class="bc-body">'+
        cmdLine+
        '<div class="bc-poke-layout">'+
          (cd.image ? '<div class="bc-poke-img-wrap" style="background:radial-gradient(circle,'+(cd.mainColor||'#5B8EF4')+'22 0%,transparent 70%)"><img class="bc-poke-img" src="'+escapeHtml(cd.image)+'" alt="'+escapeHtml(cd.name)+'"></div>' : '')+
          '<div class="bc-poke-info">'+
            '<div class="bc-poke-num">#'+String(cd.id||0).padStart(3,'0')+'</div>'+
            '<div class="bc-anime-title">'+escapeHtml(cd.name||'')+'</div>'+
            '<div class="bc-tags-row">'+typeChips+'</div>'+
            '<div class="bc-poke-dims">↕️ '+cd.height+'m &nbsp; ⚖️ '+cd.weight+'kg</div>'+
          '</div>'+
        '</div>'+
        (cd.desc ? '<div class="bc-synopsis">'+escapeHtml(cd.desc)+'</div>' : '')+
        '<div class="bc-poke-stats">'+
          statBar('PV',cd.hp,'#22C55E')+
          statBar('ATQ',cd.atk,'#EF4444')+
          statBar('DEF',cd.def,'#3B82F6')+
          statBar('ATQ.SP',cd.spatk,'#EC4899')+
          statBar('DEF.SP',cd.spdef,'#8B5CF6')+
          statBar('VIT',cd.speed,'#F59E0B')+
        '</div>'+
      '</div></div>';
    }
    ...
    if (ct === 'film') {
      var genres = (cd.genres||[]).map(function(g){ return '<span class="bc-tag">'+escapeHtml(g)+'</span>'; }).join('');
      return '<div class="bot-card"><div class="bc-stripe" style="background:linear-gradient(90deg,#F59E0B,#EF4444)"></div><div class="bc-body">'+
        cmdLine+
        '<div class="bc-anime-layout">'+
          (cd.image ? '<div class="bc-anime-poster"><img class="bc-anime-cover" src="'+escapeHtml(cd.image)+'" loading="lazy" alt="affiche"></div>' : '')+
          '<div class="bc-anime-info">'+
            '<div class="bc-anime-title">'+escapeHtml(cd.title||'')+'</div>'+
            (cd.year ? '<span class="bc-badge bc-badge-dim">📅 '+cd.year+'</span>' : '')+
            (cd.score ? '<div class="bc-score-row"><span class="bc-score-num">⭐ '+cd.score+'</span><span class="bc-score-sub">/10</span></div>' : '')+
            '<div class="bc-tags-row">'+genres+'</div>'+
            (cd.directors && cd.directors.length ? '<div class="bc-anime-subtitle">🎥 '+escapeHtml(cd.directors.join(', '))+'</div>' : '')+
          '</div>'+
        '</div>'+
        (cd.synopsis ? '<div class="bc-synopsis">'+escapeHtml(cd.synopsis)+'</div>' : '')+
        '<div class="bc-footer">'+
          (cd.runtime ? '<span>⏱ '+cd.runtime+' min</span>' : '')+
          (cd.url ? '<a class="bc-link" href="'+escapeHtml(cd.url)+'" target="_blank" rel="noopener">TMDb →</a>' : '')+
        '</div>'+
      '</div></div>';
    }
    ...
    if (ct === 'blague') {
      return '<div class="bot-card"><div class="bc-stripe" style="background:linear-gradient(90deg,#F59E0B,#EF4444)"></div><div class="bc-body">'+
        cmdLine+
        (cd.setup ? '<div class="bc-blague-setup">'+escapeHtml(cd.setup)+'</div>' : '')+
        '<div class="bc-blague-punchline">'+escapeHtml(cd.punchline||'')+'</div>'+
        (cd.category ? '<div class="bc-fact-cat">'+escapeHtml(cd.category)+'</div>' : '')+
      '</div></div>';
    }

    if (ct === 'citation') {
      return '<div class="bot-card"><div class="bc-stripe" style="background:linear-gradient(90deg,#6366F1,#8B5CF6)"></div><div class="bc-body">'+
        cmdLine+
        '<div class="bc-citation">“'+escapeHtml(cd.text||'')+'”</div>'+
        '<div class="bc-citation-author">— '+escapeHtml(cd.author||'')+'</div>'+
      '</div></div>';
    }

    if (ct === 'pays') {
      return '<div class="bot-card"><div class="bc-stripe" style="background:linear-gradient(90deg,#22C55E,#3B82F6)"></div><div class="bc-body">'+
        cmdLine+
        '<div class="bc-pays-header">'+
          (cd.flag ? '<img class="bc-pays-flag" src="'+escapeHtml(cd.flag)+'" alt="drapeau">' : '')+
          '<div class="bc-anime-title">'+escapeHtml(cd.name||'')+'</div>'+
        '</div>'+
        '<div class="bc-pays-grid">'+
          (cd.capital ? '<div class="bc-pays-item">🏛️<span>'+escapeHtml(cd.capital)+'</span></div>' : '')+
          (cd.population ? '<div class="bc-pays-item">👥<span>'+escapeHtml(cd.population)+'</span></div>' : '')+
          (cd.area ? '<div class="bc-pays-item">🌏<span>'+escapeHtml(cd.area)+'</span></div>' : '')+
          (cd.region ? '<div class="bc-pays-item">🗺️<span>'+escapeHtml(cd.region+(cd.subregion?' · '+cd.subregion:''))+'</span></div>' : '')+
          (cd.languages ? '<div class="bc-pays-item">🗣️<span>'+escapeHtml(cd.languages)+'</span></div>' : '')+
          (cd.currency ? '<div class="bc-pays-item">💰<span>'+escapeHtml(cd.currency)+'</span></div>' : '')+
          (cd.timezone ? '<div class="bc-pays-item">⏰<span>'+escapeHtml(cd.timezone)+'</span></div>' : '')+
        '</div>'+
      '</div></div>';
    }

    if (ct === 'lyrics') {
      var lyricsHtml = escapeHtml(cd.lyrics||'').replace(/\n/g, '<br>');
      return '<div class="bot-card"><div class="bc-stripe" style="background:linear-gradient(90deg,#EC4899,#F97316)"></div><div class="bc-body">'+
        cmdLine+
        '<div style="display:flex;align-items:center;gap:10px;margin-bottom:10px">'+
          '<div style="font-size:32px">🎵</div>'+
          '<div><div class="bc-anime-title">'+escapeHtml(cd.title||'')+'</div><div class="bc-anime-sub">'+escapeHtml(cd.artist||'')+'</div></div>'+
        '</div>'+
        '<div style="font-size:12px;line-height:1.9;color:rgba(255,255,255,0.82);background:rgba(255,255,255,0.04);border-radius:10px;padding:12px 14px;max-height:300px;overflow-y:auto;border-left:2px solid #EC4899">'+lyricsHtml+'</div>'+
      '</div></div>';
    }

    if (ct === 'cocktail') {
      var ings = (cd.ingredients||[]).map(function(i){ return '<span class="bc-tag">'+escapeHtml(i)+'</span>'; }).join('');
      return '<div class="bot-card"><div class="bc-stripe" style="background:linear-gradient(90deg,#F97316,#EF4444)"></div><div class="bc-body">'+
        cmdLine+
        '<div class="bc-anime-inner">'+
          (cd.image?'<img src="'+escapeHtml(cd.image)+'" style="width:75px;height:90px;object-fit:cover;border-radius:10px;flex-shrink:0" loading="lazy">':'')+
          '<div class="bc-anime-info">'+
            '<div class="bc-anime-title">'+escapeHtml(cd.name||'')+'</div>'+
            '<div class="bc-tags" style="margin:4px 0">'+
              (cd.alcoholic?'<span class="bc-badge bc-badge-type">'+escapeHtml(cd.alcoholic)+'</span>':'')+
              (cd.category?'<span class="bc-badge bc-badge-dim">'+escapeHtml(cd.category)+'</span>':'')+
            '</div>'+
            (cd.glass?'<div class="bc-anime-sub">🥃 '+escapeHtml(cd.glass)+'</div>':'')+
          '</div>'+
        '</div>'+
        '<div class="bc-tags" style="margin:8px 0 6px">'+ings+'</div>'+
        (cd.instructions?'<div class="bc-anime-synopsis" style="-webkit-line-clamp:3;display:-webkit-box;overflow:hidden">'+escapeHtml(cd.instructions)+'</div>':'')+
      '</div></div>';
    }

    if (ct === 'concert') {
      var evHtml = (cd.events||[]).map(function(e,i) {
        return '<div style="padding:8px 0;border-bottom:1px solid rgba(255,255,255,0.06);display:flex;gap:10px;align-items:flex-start">'+
          '<div style="font-size:18px;flex-shrink:0">'+(i===0?'🎤':'🎸')+'</div>'+
          '<div>'+
            '<div style="font-size:13px;font-weight:700;color:var(--text-primary,#F0F0FA)">'+escapeHtml(e.artist)+'</div>'+
            '<div style="font-size:11px;color:var(--text-secondary,#9898C0)">📍 '+escapeHtml(e.venue)+'</div>'+
            (e.date?'<div style="font-size:10px;color:var(--text-muted,#5C5C80)">📅 '+escapeHtml(e.date)+'</div>':'')+
          '</div>'+
          (e.url?'<a href="'+escapeHtml(e.url)+'" target="_blank" rel="noopener" style="margin-left:auto;font-size:10px;color:#5B8EF4;flex-shrink:0">Billets →</a>':'')+
        '</div>';
      }).join('');
      return '<div class="bot-card"><div class="bc-stripe" style="background:linear-gradient(90deg,#8B5CF6,#EC4899)"></div><div class="bc-body">'+
        cmdLine+
        '<div style="font-size:14px;font-weight:800;color:var(--text-primary,#F0F0FA);margin-bottom:10px">🎪 Concerts à '+escapeHtml(cd.city||'')+'</div>'+
        evHtml+
      '</div></div>';
    }

    if (ct === 'artiste') {
      return '<div class="bot-card"><div class="bc-stripe" style="background:linear-gradient(90deg,#F59E0B,#EC4899)"></div><div class="bc-body">'+
        cmdLine+
        '<div class="bc-anime-title" style="font-size:16px">🎤 '+escapeHtml(cd.name||'')+'</div>'+
        '<div class="bc-tags" style="margin:6px 0">'+
          (cd.type?'<span class="bc-badge bc-badge-type">'+escapeHtml(cd.type)+'</span>':'')+
          (cd.country?'<span class="bc-badge bc-badge-dim">'+escapeHtml(cd.country)+'</span>':'')+
          (cd.beginYear?'<span class="bc-badge bc-badge-dim">Depuis '+escapeHtml(cd.beginYear)+'</span>':'')+
        '</div>'+
        (cd.disambiguation?'<div class="bc-anime-sub" style="margin-bottom:8px">'+escapeHtml(cd.disambiguation)+'</div>':'')+
        '<div class="bc-tags">'+((cd.genres||[]).map(function(g){ return '<span class="bc-tag">'+escapeHtml(g)+'</span>'; }).join(''))+'</div>'+
        (cd.url?'<div style="margin-top:10px"><a href="'+escapeHtml(cd.url)+'" target="_blank" rel="noopener" style="font-size:11px;color:#5B8EF4">MusicBrainz →</a></div>':'')+
      '</div></div>';
    }
    ...
    if (ct === 'animequote') {
      return '<div class="bot-card"><div class="bc-stripe" style="background:linear-gradient(90deg,#F59E0B,#EF4444)"></div><div class="bc-body">'+
        cmdLine+
        '<div style="font-size:14px;font-style:italic;line-height:1.7;color:rgba(255,255,255,0.9);margin:8px 0 12px;padding:10px 12px;background:rgba(245,158,11,0.08);border-radius:8px;border-left:3px solid #F59E0B">"'+escapeHtml(cd.quote||'')+'"</div>'+
        '<div style="text-align:right">'+
          '<span style="font-size:12px;font-weight:700;color:#F59E0B">— '+escapeHtml(cd.character||'')+'</span>'+
          (cd.anime?'<span style="font-size:11px;color:var(--text-muted,#5C5C80)"> · '+escapeHtml(cd.anime)+'</span>':'')+
        '</div>'+
      '</div></div>';
    }
    ...
    if (ct === 'anifact') {
      return '<div class="bot-card"><div class="bc-stripe" style="background:linear-gradient(90deg,#6366F1,#8B5CF6)"></div><div class="bc-body">'+
        ...
        '<div class="bc-fact-txt">'+escapeHtml(cd.fact||'')+'</div>'+
      '</div></div>';
    }
