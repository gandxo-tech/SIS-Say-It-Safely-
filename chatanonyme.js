...
(function () {
  ...
  function makeAvatarEl(pseudo, emoji, color, size) {
    var div = document.createElement('div');
    size = size || 40;
    div.style.cssText = 'width:'+size+'px;height:'+size+'px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:'+(size*0.45)+'px;background:'+color+';flex-shrink:0;overflow:hidden;';
    if (emoji) {
  ...
  function avatarHtml(pseudo, emoji, color, size) {
    size = size || 40;
    var fs = Math.round(size * 0.45);
    var char = emoji || (pseudo || '?').charAt(0).toUpperCase();
    return '<div style="width:'+size+'px;height:'+size+'px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:'+fs+'px;background:'+(color||'#5B8EF4')+';flex-shrink:0;overflow:hidden;">' + escapeHtml(char) + '</div>';
  }

  function certBadgeHtml() {
    return '<span class="badge-certified" title="Compte officiel SIS">' +
      '<svg viewBox="0 0 20 20" width="14" height="14">' +
      '<defs><linearGradient id="certG2" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#5B8EF4"/><stop offset="100%" stop-color="#8B5CF6"/></linearGradient></defs>' +
      '<circle cx="10" cy="10" r="10" fill="url(#certG2)"/>' +
      '<polyline points="5,10 8.5,13.5 15,7" fill="none" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>' +
      '</svg></span>';
  }
  ...
  function onUserSignedIn(user) {
    ...
    db.collection('users').doc(user.uid).get()
      .then(function(doc) {
        ...
        var geo = window.SIS_GEO || {};
        var update = { status: 'online', lastSeen: firebase.firestore.FieldValue.serverTimestamp() };
  ...
  function renderUserUI() {
    ...
    if (tba) {
      tba.innerHTML = '';
      tba.appendChild(makeAvatarEl(pseudo, emoji, color, 32));
    }
    ...
    if (bna) {
      bna.innerHTML = '';
      var aEl = makeAvatarEl(pseudo, emoji, color, 28);
      bna.appendChild(aEl);
    }
    ...
    if (profAv) {
      profAv.innerHTML = '';
      profAv.appendChild(makeAvatarEl(pseudo, emoji, color, 72));
    }
    var profName = $('profileName');
    if (profName) profName.textContent = pseudo;
    var profCert = $('profileCertBadge');
    if (profCert) { profCert.style.display = d.certified ? '' : 'none'; }
    ...
    if (smAv) { smAv.innerHTML = ''; smAv.appendChild(makeAvatarEl(pseudo, emoji, color, 46)); }
  ...
  function trackPresence() {
    ...
    /* Mettre hors ligne à la fermeture */
  ...
  function showView(name) {
    ...
    /* Marquer notifs comme lues si on ouvre l'onglet */
  ...
  function initRoomsView() {
    ...
    if (btnCreate) btnCreate.addEventListener('click', function(){ show($('modalCreateRoom')); });
    if (btnCreateEmpty) btnCreateEmpty.addEventListener('click', function(){ show($('modalCreateRoom')); });
  ...
  function updateOnlineFooter() {
    ...
    keys.forEach(function(uid) {
      ...
      av.innerHTML = avatarHtml(u.pseudo, u.emoji, u.color, 28);
      av.addEventListener('click', function(){ openMemberProfile(uid, u); });
      row.appendChild(av);
    });
  }
      ...
      btnConfirm.addEventListener('click', function() {
        ...
        if (photoUrl && photoUrl.startsWith('data:')) {
          uploadToCloudinary(photoUrl, function(url) {
            createRoom(name, cat, type, type==='private'?pwd:'', url||'', color);
          });
        } else {
          createRoom(name, cat, type, type==='private'?pwd:'', photoUrl, color);
        }
      });
    }
  ...
  function createRoom(name, cat, type, pwd, photo, color) {
    db.collection('rooms').add({
      ...
      openRoom(ref.id, { name:name, category:cat, type:type, photo:photo, color:color });
    }).catch(function(err) {
      console.error(err);
  ...
  function loadMessages(roomId) {
    ...
    messagesListenerOff = db.collection('messages')
      .where('roomId', '==', roomId)
      .where('deleted', '!=', true)
      .orderBy('deleted')
  ...
  function buildMessageHtml(msgId, data, isOut, compact) {
    ...
    if (data.type === 'image') {
      ...
      content = '<img class="bubble-image" src="'+escapeHtml(data.gifUrl||'')+'" alt="gif" loading="lazy" style="max-width:200px;">';
    } else {
    ...
    var tickHtml = '';
    if (isOut) {
      tickHtml = '<span class="tick-double' + (data.readBy ? ' tick-read' : '') + '">' +
    ...
    var avatarHtmlStr = compact ? '<div class="msg-avatar"></div>' :
      '<div class="msg-avatar" data-uid="'+escapeHtml(data.authorId||'')+'">'+avatarHtml(author, emoji, color, 28)+'</div>';
  ...
  function buildPollHtml(msgId, data) {
    ...
    var isAdmin = currentUserData.role === 'admin' || currentUserData.role === 'mod';
    var optHtml = '';
  ...
  function bindReactionChips(wrap, msgId) {
    ...
    if (playBtn) {
      playBtn.addEventListener('click', function() {
        ...
        playBtn.addEventListener('click', function pauseFn() {
          audio.pause();
          if (playI) playI.style.display = '';
          if (pauseI) pauseI.style.display = 'none';
          playBtn.removeEventListener('click', pauseFn);
        });
      });
    }
  }
  ...
  function initInputBar() {
    ...
    if (btnEmoji) {
      btnEmoji.addEventListener('click', function(e) {
        e.stopPropagation();
        hide(attachMenu);
        emojiPicker.style.display = emojiPicker.style.display === 'none' ? '' : 'none';
        if (emojiPicker.style.display !== 'none') buildEmojiGrid($('emojiGrid'), msgInput);
      });
    }
    ...
    if (btnAttach) {
      btnAttach.addEventListener('click', function(e) {
        e.stopPropagation();
        hide(emojiPicker);
        attachMenu.style.display = attachMenu.style.display === 'none' ? '' : 'none';
      });
    }
  ...
  function sendMessage(payload) {
    ...
    /* Chiffrement si texte */
    if (msg.type === 'text' && encryptionKey) {
      encryptMsg(msg.text, function(err, encrypted) {
        msg.text = encrypted;
        msg.encrypted = true;
        saveMessage(msg);
      });
  ...
  function saveMessage(msg) {
    db.collection('messages').add(msg)
      .then(function() {
        ...
        var preview = msg.type === 'text' ? (msg.text||'').substr(0,60) : '📷 Média';
        db.collection('rooms').doc(currentRoomId).update({
          lastMessage:   preview,
          lastMessageAt: firebase.firestore.FieldValue.serverTimestamp()
        }).catch(function(){});
  ...
  function openMsgActions(msgId, data, isOut) {
    ...
    actionMsgData = data;
    var isAdmin = currentUserData.role === 'admin' || currentUserData.role === 'mod';
    /* Afficher/masquer boutons selon droits */
    var actionEdit   = $('actionEdit');
    var actionDelete = $('actionDelete');
    var actionPin    = $('actionPin');
    if (actionEdit)   actionEdit.style.display   = isOut ? '' : 'none';
    if (actionDelete) actionDelete.style.display = (isOut || isAdmin) ? '' : 'none';
    if (actionPin)    actionPin.style.display     = isAdmin ? '' : 'none';
    ...
    if (actionDelete) actionDelete.onclick = function() { deleteMessage(msgId, isOut || isAdmin); hide($('modalMsgActions')); };
  ...
  function toggleReaction(msgId, emoji) {
    ...
    db.collection('messages').doc(msgId).get().then(function(doc) {
      ...
      if (reactions[uid] === emoji) {
        /* Retirer */
        db.collection('messages').doc(msgId).update(update).catch(function(){});
      } else {
        /* Ajouter */
        var add = {};
        add['reactions.' + uid] = emoji;
        db.collection('messages').doc(msgId).update(add).catch(function(){});
      }
    });
  }
  ...
  function deleteMessage(msgId, allowed) {
    if (!allowed) return;
    db.collection('messages').doc(msgId).update({ deleted: true })
  ...
  function startEditMessage(msgId, data) {
    ...
    input.focus();
    /* Replace send button behavior */
    var btnSend = $('btnSend');
    if (btnSend) {
      btnSend.style.display = '';
      var btnVoice = $('btnVoice');
      if (btnVoice) btnVoice.style.display = 'none';
      var originalClick = btnSend.onclick;
  ...
  function uploadVoiceToCloudinary(dataUrl, cb) {
    uploadToCloudinary(dataUrl, cb, 'video');
  }

  /* ══════════════════════════════════════════════════════════════
     16. UPLOAD CLOUDINARY
  ══════════════════════════════════════════════════════════════ */
  function uploadToCloudinary(dataUrl, cb, resourceType) {
    resourceType = resourceType || 'image';
    showToast('Envoi en cours…','info',10000);
    var xhr = new XMLHttpRequest();
    xhr.open('POST', 'https://api.cloudinary.com/v1_1/'+CLOUDINARY_NAME+'/'+resourceType+'/upload', true);
    xhr.onload = function() {
      if (xhr.status === 200) {
        try {
          var data = JSON.parse(xhr.responseText);
          showToast('Envoyé !','success',1500);
          cb(data.secure_url || data.url || null);
  ...
  function closePoll(msgId) {
    db.collection('messages').doc(msgId).update({ closed: true })
  ...
  function openRoomInfo(roomId, roomData) {
    ...
    if (rMeta)  rMeta.innerHTML = '<span>📁 '+escapeHtml(d.category||'general')+'</span>' +
      '<span>'+((d.type==='private') ? '🔒 Privé' : '🌐 Public')+'</span>' +
      '<span>👥 '+((d.membersOnline||0))+' membres</span>';

    /* Médias */
    var mediaGrid = $('roomMediaGrid');
    if (mediaGrid) {
      mediaGrid.innerHTML = '';
      db.collection('messages').where('roomId','==',roomId).where('type','==','image').orderBy('createdAt','desc').limit(12).get()
        .then(function(snap) {
    ...
    /* Slow mode (admin/mod seulement) */
    var slowCtrl = $('slowModeControl');
    if (slowCtrl) {
      var isAdmin = currentUserData.role === 'admin' || currentUserData.role === 'mod' || d.createdBy === currentUser.uid;
      slowCtrl.style.display = isAdmin ? '' : 'none';
      if (isAdmin) {
        var slider = $('slowModeSlider');
        var sliderVal = $('slowModeValue');
        if (slider) {
          slider.value = d.slowMode || 0;
          if (sliderVal) sliderVal.textContent = (d.slowMode||0)+'s';
          slider.oninput = function() {
            if (sliderVal) sliderVal.textContent = slider.value+'s';
          };
          slider.onchange = function() {
            db.collection('rooms').doc(roomId).update({ slowMode: parseInt(slider.value,10) })
  ...
  function openRoomMembers(roomId) {
    ...
    db.collection('users').where('status','in',['online','away']).limit(50).get().then(function(snap) {
      ...
      snap.forEach(function(doc) {
        ...
        item.innerHTML =
          '<div class="member-avatar">' + avatarHtml(u.pseudo, u.emoji, u.color, 40) + '</div>' +
    ...
    /* Recherche membres */
    var memberSearch = $('membersSearch');
    if (memberSearch) {
  ...
  function openMemberProfile(uid, userData) {
    ...
    if (av) { av.innerHTML = ''; av.appendChild(makeAvatarEl(u.pseudo, u.emoji, u.color, 64)); }
  ...
  function loadDmList() {
    if (dmsListenerOff) dmsListenerOff();
    var list = $('dmList');
    if (!list) return;
    ...
    dmsListenerOff = db.collection('dms')
      ...
      .onSnapshot(function(snap) {
        list.innerHTML = '';
        if (snap.empty) {
          list.innerHTML = '<div class="empty-dms"><svg viewBox="0 0 32 32" fill="none" width="36" height="36"><path d="M16 3C8.82 3 3 8.82 3 16c0 2.6.75 5.03 2.05 7.08L3 29l6.13-1.99A12.9 12.9 0 0016 29c7.18 0 13-5.82 13-13S23.18 3 16 3z" stroke="var(--text-muted)" stroke-width="1.5" fill="none"/></svg><p>Aucune conversation</p><span>Ouvre le profil d\'un membre pour lui écrire</span></div>';
          return;
        }
        snap.forEach(function(doc) {
          var d = doc.data();
          ...
          db.collection('users').doc(otherUid).get().then(function(uDoc) {
            var u = uDoc.data() || {};
            var item = document.createElement('div');
            item.className = 'dm-item' + (doc.id === currentDmId ? ' active' : '');
            var unread = (d.unread && d.unread[currentUser.uid]) || 0;
            item.innerHTML =
              '<div class="dm-item-avatar"><div class="dm-item-avatar-img">' + avatarHtml(u.pseudo, u.emoji, u.color, 48) + '</div>' +
              '<div class="dm-item-status" style="background:'+statusColor(u.status)+'"></div></div>' +
              '<div class="dm-item-info">' +
                '<div class="dm-item-top"><span class="dm-item-name">' + escapeHtml(u.pseudo||'Anonyme') + '</span><span class="dm-item-time">' + escapeHtml(fmtRelTime(d.lastMessageAt)) + '</span></div>' +
                '<div class="dm-item-preview">' + escapeHtml((d.lastMessage||'').substr(0,40)) + '</div>' +
              '</div>' +
              (unread > 0 ? '<div class="dm-unread">'+unread+'</div>' : '');
            item.addEventListener('click', function(){ openDm(otherUid, u, doc.id); });
            list.appendChild(item);
          });
        });
  ...
  function openDm(otherUid, otherUser, existingDmId) {
    ...
    if (dmAv) { dmAv.innerHTML = ''; dmAv.appendChild(makeAvatarEl(otherUser.pseudo, otherUser.emoji, otherUser.color, 38)); }
    if (dmName) dmName.textContent = otherUser.pseudo || 'Anonyme';
    if (dmStat) {
      var statLabels = { online:'En ligne', away:'Absent', dnd:'Occupé', offline:'Hors ligne' };
      dmStat.innerHTML = '<div style="width:6px;height:6px;border-radius:50%;background:'+statusColor(otherUser.status)+';"></div>' + (statLabels[otherUser.status]||'Hors ligne');
    }
    show($('dmChatArea'));
    ...
    if (existingDmId) {
      ...
      loadDmMessages(existingDmId);
    } else {
      /* Chercher si DM existe déjà */
      var dmRef = db.collection('dms')
        .where('members', 'array-contains', currentUser.uid);
    ...
    /* Bouton retour DM */
    var btnBackDm = $('btnBackDms');
    if (btnBackDm) {
      btnBackDm.onclick = function() {
    ...
    /* Input bar DM */
    var dmInput  = $('dmInput');
    var btnDmSend = $('btnDmSend');
    if (dmInput && btnDmSend) {
      btnDmSend.onclick = function() {
        var text = dmInput.textContent.trim();
        if (!text || !currentDmId) return;
        dmInput.textContent = '';
        db.collection('dms').doc(currentDmId).collection('messages').add({
          text:         text,
      ...
      dmInput.addEventListener('keydown', function(e) {
        if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); btnDmSend.click(); }
      });
    }

    /* Annuler réponse DM */
    var btnCancelDmReply = $('btnCancelDmReply');
    if (btnCancelDmReply) {
      btnCancelDmReply.onclick = function() { dmReplyToMsg = null; hide($('dmReplyBar')); };
    }
  }

  function loadDmMessages(dmId) {
    if (dmsListenerOff) { /* on garde le listener de liste */ }
    var c = $('dmMessages');
    if (c) c.innerHTML = '';
    ...
    db.collection('dms').doc(dmId).collection('messages')
      ...
      .onSnapshot(function(snap) {
        ...
        snap.forEach(function(doc) {
          ...
          var compact = lastAuthorId === d.senderId;
          lastAuthorId = d.senderId;
          var wrap = document.createElement('div');
          wrap.className = 'msg-wrap ' + (isOut ? 'out' : 'in') + (compact ? ' compact' : '');
          var timeStr = fmtTime(d.createdAt);
          var replyHtml = '';
          if (d.replyTo) {
            replyHtml = '<div class="bubble-reply"><div class="bubble-reply-author">'+escapeHtml(d.replyTo.senderPseudo||'')+'</div><div class="bubble-reply-text">'+escapeHtml((d.replyTo.text||'').substr(0,80))+'</div></div>';
          }
          wrap.innerHTML =
            (isOut ? '' : '<div class="msg-author">'+escapeHtml(d.senderPseudo||'')+'</div>') +
            '<div class="msg-row">' +
              (isOut ? '' : '<div class="msg-avatar">'+avatarHtml(d.senderPseudo,d.senderEmoji,d.senderColor,28)+'</div>') +
              '<div class="bubble">'+replyHtml+'<div class="bubble-text">'+parseMarkdown(d.text||'')+'</div>' +
  ...
  function initRandomView() {
    ...
    var btnCancelSearch = $('btnCancelSearch');
    if (btnCancelSearch) btnCancelSearch.addEventListener('click', cancelRandomSearch);
    var btnSkip  = $('btnSkipRandom');
    if (btnSkip)  btnSkip.addEventListener('click', skipRandom);
    var btnLeave = $('btnLeaveRandom');
    if (btnLeave) btnLeave.addEventListener('click', leaveRandom);

    /* Input aléatoire */
    var randInput = $('randomInput');
  ...
  function startRandomSearch() {
    ...
    cancelRandomSearch = null;

    /* Chercher quelqu'un qui attend */
    db.collection('randomQueue')
      ...
      .then(function(snap) {
        if (!snap.empty) {
          ...
          /* Écouter quand un session est créée avec nous */
          if (randomListenerOff) randomListenerOff();
          randomListenerOff = db.collection('randomSessions')
  ...
  function showRandomChat(partnerId, partnerData, sessionId) {
    ...
    if (av)   { av.innerHTML = ''; av.appendChild(makeAvatarEl(partnerData.pseudo, partnerData.emoji, partnerData.color || '#5B8EF4', 44)); }
  ...
  function updateNotifBadge(count) {
    var bnavBadge = $('bnavNotifBadge');
    var topBadge  = $('topbarNotifBadge');
    ...
    if (notifCount) notifCount.textContent = count > 0 ? count : '';
  }
  ...
  function incrementUnread(roomId) {
    ...
    /* Badge bnav salons */
    var bnavBadgeRooms = $('bnavRoomsBadge');
          ...
          if (change.type === 'added') {
            ...
            try {
              if (!localStorage.getItem(key)) {
                showToast('📢 ' + (d.text||'Annonce'), 'info', 8000);
  ...
  function initProfileView() {
    ...
    /* Avatar */
    var btnAvatarEdit = $('profileAvatarEdit');
    var fileAvatar = $('profileAvatarFile');
    ...
    /* Déconnexion */
    var btnLogout = $('btnProfileLogout');
  ...
  function initAdminTabs() {
    qsa('.admin-tab').forEach(function(tab) {
      tab.addEventListener('click', function() {
        ...
        tab.classList.add('active');
        var panel = $('adminPanel' + tab.dataset.admintab.charAt(0).toUpperCase() + tab.dataset.admintab.slice(1));
        if (panel) { panel.classList.add('active'); loadAdminTab(tab.dataset.admintab); }
      });
    });
    ...
    if (tab === 'users') {
      ...
      db.collection('users').orderBy('createdAt','desc').limit(30).get().then(function(snap) {
        ...
        snap.forEach(function(doc) {
          ...
          item.innerHTML =
            '<div class="admin-user-avatar">' + avatarHtml(u.pseudo, u.emoji, u.color, 36) + '</div>' +
            '<div class="admin-user-info"><div class="admin-user-name">' + escapeHtml(u.pseudo||'Anonyme') + (u.banned ? ' 🚫' : '') + '</div><div class="admin-user-role">' + (u.role||'user') + ' · ' + (u.email||'anon') + '</div></div>' +
            '<div class="admin-actions">' +
              '<button class="icon-btn icon-btn-sm" title="' + (u.banned ? 'Débannir' : 'Bannir') + '" data-uid="'+uid+'" data-banned="'+(u.banned?'1':'0')+'">' +
                (u.banned
              ...
              '</button>' +
            '</div>';
          var banBtn = item.querySelector('[data-uid]');
          if (banBtn) {
    ...
    Promise.all([roomsPromise, usersPromise]).then(function(values) {
      ...
      if (!userSnap.empty) {
        ...
        userSnap.forEach(function(doc) {
          ...
          item.className = 'search-result-item';
          item.innerHTML =
            avatarHtml(u.pseudo, u.emoji, u.color, 36) +
  ...
  function buildEmojiGrid(grid, targetInput) {
    if (!grid) return;
    if (grid.innerHTML) return; /* Déjà construit */
  ...
  function boot() {
    ...
    /* Initialiser les modales de fermeture globale */
    document.addEventListener('keydown', function(e) {
      if (e.key === 'Escape') {
        /* Fermer la modale la plus haute ouverte */
        var overlays = Array.from(document.querySelectorAll('.overlay:not([style*="display: none"])'));
        if (overlays.length) { overlays.forEach(function(o){ hide(o); }); return; }
        /* Fermer la sidebar mobile si ouverte */
        var sideMenu = $('sideMenu');
        if (sideMenu && sideMenu.style.display !== 'none') {
          hide(sideMenu);
          hide($('sideMenuOverlay'));
          return;
        }
        /* Revenir au chat → fermer sur mobile */
        if (currentRoomId && window.innerWidth < 640) { closeRoom(); }
      }
    });
    ...
    if (modalSearch) {
      ...
      var observer = new MutationObserver(function() {
        if (modalSearch.style.display !== 'none' && firstShow) {
          firstShow = false;
          initGlobalSearch();
        }
      });
