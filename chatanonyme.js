  ...
  Promise.all(promises).then(function(decryptedMsgs) {
    ...
    decryptedMsgs.forEach(function(msg) {
      ...
      html += '<div class="msg-row' + (isOwn ? ' own' : '') + (msg.pinned ? ' pinned-msg' : '') + '" id="msg_' + msg.id + '">' +
        (!isOwn ? '<div class="msg-avatar-wrap" onclick="openPublicProfile(\'' + msg.uid + '\')" title="Voir le profil">' + avatarHtml.replace('<div class="msg-avatar-wrap">', '').replace('</div>', '') + '</div>' : avatarHtml) +
...
function openGifPanel() {
  ...
  if (gifState.open) {
    ...
    searchGifs('meme'); // trending par défaut
